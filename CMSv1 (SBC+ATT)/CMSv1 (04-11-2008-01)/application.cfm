...
<cfset Request.product_metaTags_args = 'author=#URLEncodedFormat("&copy;2005-#dateformat(now(), "yyyy")#, Hierarchical Applications Limited - All Rights Reserved")#,keywords=,description=#URLEncodedFormat("Web Based Content Management System v1. No part of this product may be used, reproduced or distributed without the expressed conscent, in writing, of Hierarchical Applications Limited or a duly authorized agent of same.")#,robots=all,category=home page'>
...
<cfscript>
	path_prefix = Request.commonCode.fullyQualifiedURLprefix(Request.commonCode.scriptPrefix(CGI.SCRIPT_NAME));
</cfscript>

<cftry>
	<!--System application.cfm-->
	<cfinclude template="#path_prefix#Application.cfm">
...
<cftry>
	<cfinclude template="#path_prefix#errhandler.cfm">
...
<cfscript>
	// +++ TO-DO:  Determine if the database is empty and fill it as-needed to ensure the data model exists.
	// Code some T-SQL to handle this - test for each table and create if a table don't exist...
</cfscript>
