var bool_disable_focus = false;

const_anchor_open = '<A';
const_anchor_close = '</A>';
const_object_symbol = 'object';
const_href_symbol = 'href';

function getAnchorParms(a) {
	var aa = [];
	var t = typeof a;
	if (t.toUpperCase() == const_object_symbol.toUpperCase()) {
		for (var i = 0; i < a.length; i++) {
			var ba = a[i].split('=');
			if (ba.length > 1) {
				var x = '';
				for (var k = 1; k < ba.length; k++) {
					x += ba[k] + ((k != (ba.length - 1)) ? '=' : '');
				}
				var bb = x.split('"');
				aa[ba[0].trim().toUpperCase()] = bb[((bb.length > 1) ? 1 : 0)];
			}
		}
	}
	return aa;
...
function handleCustomWebLinkEditor(editdoc, objname, cmdID) {
	var sRange  = editdoc.selection.createRange();
    var sHtml   = sRange.htmlText;
	var p = '';
	var h = '';

	var const_warning_text = 'Possibly unable to determine the link you wish to edit... \nHave you highlighted the whole link ?\nHave you highlighted more than one link ?\nPlease highlight a single link and try again.';

	var isAddingNewLink = false;
	var t_link_text = adjustLinkTempatePerUsage('[new link template]', editdoc.body.innerHTML.trim());
	var t_link = '<a href="' + const_cgi_script_name_symbol + const_currentPage_symbol + '_automatic_unique_link_' + '">' + t_link_text + '</a>';

	var const_warning_text2 = 'Please highlight the text "' + t_link_text + '" and try again.  Typically this is done by typing the <shift> key and <home> key together.  After you have edited this new link you may change the text for the prompt for this link which is now "' + t_link_text + '".';

	var _bool_foundAnchorTag = false;

	function processLinkReplacementParms(s, c) {
		var _f = c.toUpperCase().indexOf(s.toUpperCase());
		if (_f != -1) {
			_f2 = c.toUpperCase().indexOf(const_anchor_close.trim().toUpperCase(), (_f + s.length));
			if (_f2 != -1) {
				_f2 += const_anchor_close.trim().length;
				// _f2 is the index of the end of the anchor tag - now find the beginning of this same tag...
				for (var i = _f; i > 0; i--) {
					_f1 = c.toUpperCase().indexOf(const_anchor_open.trim().toUpperCase(), i);
					if ( (_f1 != -1) && (_f1 < _f) ) {
						// _f1 is the index of the beginning of this anchor tag...
						var _sHtml = c.slice(_f1, _f2);
						var aa = _sHtml.trim().split('>');
						p = aa[1] + '>';				// make sure the end of the anchor tag is intact so it can be filtered off to complete the processing of the anchor's prompt sans the HTML...
						p = p.trim().stripHTML().trim(); // this is necessary because simply splitting the sliced string based on the end of the tag intriduced a bug (Duh !)...
						var _sHtml_tag = aa[0];
						// here sHtml is just the anchor HTML
						var a = getAnchorParms(_sHtml_tag.trim().split(' '));
						h = a[const_href_symbol.trim().toUpperCase()];
						_bool_foundAnchorTag = true;
						break;
					}
				}
			}
	...
	function makeLinkTemplateUsing(s, i) {
		var a = s.split(']');
		if (a.length > 0) {
			s = a[0] + ' ' + ((i > 0) ? i.toString() + ']' : '');
		}
		return s;
	}

	function adjustLinkTempatePerUsage(s, c) {
		// are there any templates at all using the base (common) part of the template ?
		// if there are any, then begin looking for other templates from '1' until there are no more that match
		var t_any = makeLinkTemplateUsing(s, -1);
		var t_f = c.toUpperCase().indexOf(t_any.toUpperCase());
		...
		if (t_f != -1) {
			...
			var t = '';
			for (var i = 1; _f != -1; i++) {
				t = makeLinkTemplateUsing(s, i);
				_f = c.toUpperCase().indexOf(t.toUpperCase());
				if (_f == -1) {
					break;
				}
			}
			s = makeLinkTemplateUsing(s, i);
		} else {
			var _f = 1;
			for (var i = 1; _f != -1; i++) {
				_f = c.toUpperCase().indexOf(s.toUpperCase());
				if (_f == -1) {
					break;
				}
			}
	...
	var a = sHtml.caselessIndexOfAll(const_anchor_open);
	if (a.length == 1) {
		var _f = a[0];
		if (_f != -1) {
			_f += const_anchor_open.length;
			var _f2 = sHtml.trim().toUpperCase().indexOf(const_anchor_close.trim().toUpperCase(), _f);
			if (_f2 != -1) {
				sHtml = sHtml.trim().slice(_f, _f2);
				var aa = sHtml.trim().split('>');
				p = aa[1];
				sHtml = aa[0];
				// here sHtml is just the anchor HTML
				var a = getAnchorParms(sHtml.trim().split(' '));
				h = a[const_href_symbol.trim().toUpperCase()];
			}
		} else if (sHtml.trim().length > 0) {
			alert(const_warning_text);
			return -1;
		}
		...
		processLinkReplacementParms(sHtml.trim(), contents.trim());

		if (_bool_foundAnchorTag == false) {
			// the solution is to insert a link template and then edit it on-the-fly without changing the textual prompts...
			editor_insertHTML(objname, t_link);
			var _f1 = -1;
			var _f2 = -1;
		    var contents = editdoc.body.innerHTML;
			processLinkReplacementParms(t_link_text.trim(), contents.trim());
			if (_bool_foundAnchorTag == false) {
				isAddingNewLink = true;
				alert(const_warning_text2);
				return -1;
			}
		}
	}

    var myTitle = 'Insert Hyperlink';
	var a_array = [];
	a_array.push(myTitle);
	a_array.push(_GetCurrentContent_pageList);
	a_array.push(_GetCurrentContent_notLinkables);
	var h_array = [];
	h_array.push(h);
	h_array.push(p);
	a_array.push(h_array);
	a_array.push(const_currentPage_symbol);
	a_array.push(const_cgi_script_name_symbol);
	var r = Math.random().toString();

	var myURL = _editor_url + "popups/insert_hyperlink.cfm?nocache=" + r;
    var myText = showModalDialog(myURL,
                       a_array,
                       "dialogHeight: 8; dialogWidth: 40; center: yes; resizable: no; help: no; status: no; scroll: no; ");
    if (myText) { 
		if (isAddingNewLink == true) {
		} else if (_bool_foundAnchorTag == true) {
			// here we have to replace the link the user edited with the one from the contents buffer...
		    editdoc.body.innerHTML = editdoc.body.innerHTML.replaceSubString(_f1, _f2, myText);
			// here we have to set mode to html then back to wysiwyg to clear the state from this operation...
			bool_disable_focus = true; // inhibit the focus change because this will happen too fast and the browser will complain...
			editor_setmode(objname); // set to html mode
			editor_setmode(objname); // set back to wysiwyg mode
			// +++
		} else {
...
//    "createlink":     ['CreateLink',           'Insert Web Link for Internal or External Page\n(highlight a link to edit or edit the new link template)\nLinks to pages of content that are linked via the site menu will not be available here because the system will not allow previously linked pages of content to be linked again unless the link was created first using the procedure provided by this icon.',    'editor_action(this.id)',  'ed_link.gif'],
    ...
    "custom2":           ['custom2',         'Insert Web Link for Internal or External Page\n(highlight a link to edit)',  'editor_action(this.id)',  'ed_custom.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      // fontname
      if (btnName == "fontname") {
        toolbar += '<select id="_' +objname+ '_FontName" title="Choose a Font from this list." onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 2; font-size: 10px; z-index: 1;">';
      ...
      // fontsize
      if (btnName == "fontsize") {
        toolbar += '<select id="_' +objname+ '_FontSize" title="Choose a Font Size from this list." onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; font-size: 10px; z-index: 1;">';
      ...
      if (btnName == "fontstyle") {
        toolbar += '<select id="_' +objname+ '_FontStyle" title="Choose a Font Style from this list." onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; font-size: 10px; z-index: 1;">';
  ...
  if (config.plaintextInput) {     // replace nextlines with breaks
	...
	var divObj = document.getElementById('_' + objname + '_html');
	if (divObj != null) {
		divObj.innerHTML = document.all[objname].value;
		_autoCorrectLinksAndImages(_autoCorrect_context, _autoCorrect_images_context, divObj.id);
//		alert('_autoCorrect_context = (' + _autoCorrect_context + '), _autoCorrect_images_context = (' + _autoCorrect_images_context + '), divObj.id = (' + divObj.id + ')\n' + divObj.innerHTML);
		document.all[objname].value = divObj.innerHTML;
	}
...
//	  handleCustomWebLinkEditor(editdoc, objname, cmdID);
    }
...
function editor_updateHTMLmode(objname,action) {
  var config = document.all[objname].config;
  var editor_obj  = document.all["_" +objname+  "_editor"];
