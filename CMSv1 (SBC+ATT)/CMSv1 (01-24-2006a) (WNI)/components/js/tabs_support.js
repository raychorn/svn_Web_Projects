...
function giveStatusForOpenItems(_a) {
	...
	var statObj = getGUIObjectInstanceById('open_process_indicator');
	if (statObj != null) {
...
function _disableAllButtonsInContentAreaForTab(_num, exceptThisButtonValue, for_handle) {
	var obj = getGUIObjectInstanceById(_const__content + _num);
	if (obj != null) {
		var cbObj = -1;
...
function FocusOnThisTab(i) {
	...
	if ((i >= 0) && (i < len)) {
		...
		for(var j = 1; j <= len; j++) {
			var _objE = getGUIObjectInstanceById(_const_cell_tab + j.toString());
			if (_objE != null) {
		...
		for(var j = _vis_tabs_begin; j <= _vis_tabs_end; j++) {
			_objE = getGUIObjectInstanceById(_const_cell_tab + j.toString());
			if (_objE != null) {
...
function handle_fontSizeAdjustments_forTabs(newSize) {
	...
	var dObj = getGUIObjectInstanceById(_const__content + _num);
	if (dObj != null) {
		var a = dObj.all;
		for (var i = 0; i < a.length; i++) {
...
function _tabChanged(_num) {
	if (_allow_AutoTabsAdjustment) {
		...
		if (_num == _vis_tabs_end) {
			_objE = getGUIObjectInstanceById(_const_cell_tab + (_num + 1));
			if (_objE != null) {
				_objE.style.display = const_inline_style;
				_vis_tabs_begin++;
				_vis_tabs_end++;
				...
				_objB = getGUIObjectInstanceById(_objB_name);
				if (_objB != null) {
		...
		if (_num == _vis_tabs_begin) {
			...
			if (_objB != null) {
				...
				_objE = getGUIObjectInstanceById(_const_cell_tab + (_num + _vis_tabs_max));
				if (_objE != null) {
					_objE.style.display = const_none_style;
				}
			}
	...
	var _obj = getGUIObjectInstanceById(_const__content + _num.toString());
	if (_obj != null) {
		_obj.style.display = const_inline_style;
	}

	performCloseSBCUIDEditor(_num);
...
function setStyleForAllTabs(s, i, j) {
	for(; i <= j; i++) {
		var _obj = getGUIObjectInstanceById(_const_cell_tab + i.toString());
		if (_obj != null) {
...
function refreshTabs(ts) {
	...
	var _obj = getGUIObjectInstanceById("cell_" + _id);
	var _num = activeTabNumFrom(ts);
	var d = '';
	if (_obj != null) {
	...
	cleanUp : function(special_area_tag, special_area_func) {
		if (this._f != -1) {
			...
			if ( (cObj != null) && (isTextarea(cObj) == false) ) {
				...
				var divObj = getGUIObjectInstanceById(this.div);
