...
//
------------------------------------------------------------->
<cfset _debugIP = "127.0.0.1">

<cfparam name="variables.thisServerType" type="string" default="dev-test">
...
<cftry>
	<cfapplication name="#_cfapplication_name#"
				   ...
				   CLIENTSTORAGE="clientvars-Native"
	...
	if (FindNoCase(LCase('\go\'), LCase(CGI.PATH_TRANSLATED)) gt 0) {
		...
		switch(variables.thisServerType) {
			...
			case "dev-test": {
				...
				DSNSource = "test-cms1-ODBC";
				DSNUser = "test_cf";
				DSNPassword = "peekaboo";
			 	break;
			}
			case "go-staging": {
...
<cfif (Len(_AUTH_USER) eq 0)>
	<cfset _AUTH_USER = "rh4142">
</cfif>
