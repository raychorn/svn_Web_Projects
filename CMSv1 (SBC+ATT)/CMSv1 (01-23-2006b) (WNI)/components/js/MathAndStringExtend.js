...
//window.status = 'handle_ToolTip_MouseOver :: ' + 'id = ' + id + ', obj = ' + obj;
	if (obj != null) {
		var xPos_m = -1;
		...
		if (tt_obj != null) {
			...
			} else if ( (menuArray) && (typeof menuArray == const_object_symbol) && (menuArray.length > 0) && (xPos < 160) ) {
				xPos = 160;
			}
			if (xPos_m != -1) {
				xPos = xPos_m;
...
function disableVisibleButtons(bool, id) {
	var dObj = getGUIObjectInstanceById(id);
	if (dObj != null) {
		var b_bool = ((bool == true) ? true : false);
		var a = dObj.getElementsByTagName("INPUT");
		for (var i = 0; i < a.length; i++) {
			btnObj = a[i];
			if ( (btnObj.type.trim().toUpperCase() == const_button_symbol.trim().toUpperCase()) && (btnObj.style.display.trim().toUpperCase() == const_inline_style.trim().toUpperCase()) ) {
				btnObj.disabled = b_bool;
			}
		}
	}
}
...
//if ( (this.length > 1000) && (this.length < 2000) ) alert('_hasHTMLtags = ' + _hasHTMLtags + ', this.trim().length = ' + this.trim().length + ', _s.length = ' + _s.length);
