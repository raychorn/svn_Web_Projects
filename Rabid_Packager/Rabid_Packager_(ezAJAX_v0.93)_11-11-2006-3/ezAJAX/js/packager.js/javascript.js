...
function queueUp_AJAX_Sessions(sCmd, sArgsQueryString) {
	...
	switch (sCmd) {
		case js_const_checkLinkedObjects_symbol:
			...
			oAJAXEngine.doAJAX('performCheckGeonosisLinkedObjects', 'displayCheckedLinkedObjectsCompletion', sArgsQueryString);
		break;
		
		case js_const_getLinkedObjects_symbol:
			sArgsQueryString += '&sCmd=' + sCmd;
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performGetGeonosisLinkedObjects', 'displayLinkedObjectsBrowserCompletion', sArgsQueryString);
		break;
		
		case js_const_addAttribute_symbol:
			sArgsQueryString += '&sCmd=' + sCmd;
			oAJAXEngine.addNamedContext(js_const_addAttribute_symbol, '');
			oAJAXEngine.doAJAX('performAddGeonosisAttribute', 'displayAddOrDropAttributeCompletion', sArgsQueryString);
		break;
		
		case js_const_dropAttribute_symbol:
			sArgsQueryString += '&sCmd=' + sCmd;
			oAJAXEngine.addNamedContext(js_const_addAttribute_symbol, '');
			oAJAXEngine.doAJAX('performDropGeonosisAttribute', 'displayAddOrDropAttributeCompletion', sArgsQueryString);
		break;
		
		case js_const_dropObject_symbol:
			sArgsQueryString += '&sCmd=' + sCmd;
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performDropGeonosisObject', 'displayAddOrDropObjectCompletion', sArgsQueryString);
		break;
		
		case js_const_addObject_symbol:
			sArgsQueryString += '&sCmd=' + sCmd;
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performAddGeonosisObject', 'displayAddOrDropObjectCompletion', sArgsQueryString);
		break;
		...
		case js_const_dropClassDef_symbol:
			...
			oAJAXEngine.doAJAX('performDropGeonosisClassDef', 'displayAddOrDropClassDefCompletion', sArgsQueryString);
		break;
		...
		case js_const_addClassDef_symbol:
			...
			oAJAXEngine.doAJAX('performAddGeonosisClassDef', 'displayAddOrDropClassDefCompletion', sArgsQueryString);
		break;
		
		case js_const_chgDataForObject_symbol:
			sArgsQueryString += '&sCmd=' + sCmd;
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performSetGeonosisObjectData', 'displayObjectsBrowserEditCompletion', sArgsQueryString);
		break;
		
		case js_const_chgAttrsForObject_symbol:
			sArgsQueryString += '&sCmd=' + sCmd;
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performSetGeonosisObjectAttribute', 'displayObjectsBrowserEditCompletion', sArgsQueryString);
		break;
...
// +++
					var dragHandle = '<div id="div_dragHandle_' + cObj.options[sel].value + '_' + uuid() + '" title="This is a Draggable Object - Drag it to the Objects to Link Selection." onmousedown="handle_onmousedown_beginDrag(this, \'selection_objects2Link\'); return false;" onmouseover="handle_onmouseover_beginDrag(this); return false;" onmouseout="handle_onmouseout_beginDrag(this); return false;" style="border: thin dotted black;">' + 'Object: ' + cObj.options[sel].text + '</div>';
...
function handle_onChange_objects2Link(oSel) {
	...
	var sParms = '';
	var selectedOptions = [];
	var oBtnAdd = $('btn_objectLink_add');
	var oBtnDrop = $('btn_objectLink_drop');
		...
		if (!bool_isAnyErrorRecords) {
			...
			if (!!cObj) {
				...
				if (cObj.innerHTML.length == 0) {
					...
					_html += beginTable(_btnHtmlL + 'Objects to Link' + _btnHtmlR, '&tr=' + 'bgcolor="silver"'.URLEncode() + '&td=' + 'align="center"'.URLEncode() + '&span=' + 'class="boldPromptTextClass"'.URLEncode());
					_html += '<tr>' + '<td>';
