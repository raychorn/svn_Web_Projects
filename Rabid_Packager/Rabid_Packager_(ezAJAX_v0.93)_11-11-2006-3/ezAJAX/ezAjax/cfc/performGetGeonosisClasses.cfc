	...
	<cfscript>
		function _userDefinedAJAXFunctions(qryStruct) {
			try {
				...
				QueryAddRow(qObj, 1);
				QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
				QuerySetCell(qObj, 'errorMsg', '<font color="red"><b>Something dreadful just happened... :: Reason: "Detail=Please check that the given name is correct and that the component exists.,Message=Could not find the ColdFusion Component performGetGeonosisClasses.,missingFileName=performGetGeonosisClasses,Type=Application," [*** Unknown ***]</b></font>', qObj.recordCount);
