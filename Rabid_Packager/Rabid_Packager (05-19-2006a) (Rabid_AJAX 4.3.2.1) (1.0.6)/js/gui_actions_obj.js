...
GUIActionsObj.get$ = function() {
	// the object.id is the position within the array that holds onto the objects...
	var instance = GUIActionsObj.$[GUIActionsObj.$.length];
	if(instance == null) {
		instance = GUIActionsObj.$[GUIActionsObj.$.length] = new GUIActionsObj(GUIActionsObj.$.length);
	}
	return instance;
};

GUIActionsObj.remove$ = function(id) {
	var ret_val = false;
	if ( (id > -1) && (id < GUIActionsObj.$.length) ) {
		var instance = GUIActionsObj.$[id];
		if (!!instance) {
			GUIActionsObj.$[id] = object_destructor(instance);
			ret_val = (GUIActionsObj.$[id] == null);
		}
	}
	return ret_val;
};

GUIActionsObj._remove$ = function() {
	var ret_val = true;
	for (var i = 0; i < GUIActionsObj.$.length; i++) {
		GUIActionsObj.remove$(i);
	}
	GUIActionsObj.$ = [];
	return ret_val;
};
...
GUIActionsObj.prototype = {
	...
	push : function(id) {
		...
		if (!!oo) {
			...
			this.aspectStack.push(DictionaryObj.get$());
			this.stylesStack.push(DictionaryObj.get$());
	...
	destructor : function() {
		this.destructDicts();
		return (this.id = GUIActionsObj.$[this.id] = this.aspectStack = this.stylesStack = this.stylesDict = this.aspectDict = this.ooStack = this.stack = null);
	},
	dummy : function() {
		return false;
	}
