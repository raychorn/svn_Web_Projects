		...
		//	window.status = 'Event: ondrag !' + ' (' + draggableObj.style.left + ',' + draggableObj.style.top + ')' + ', draggableObj.id = [' + draggableObj.id + ']';
	 	...
	 	drObj.ondragdrop = function(e) {
			...
			if (!!dragTarget.options) {
				...
				if (!boolFound) {
					dragTarget.options[dragTarget.options.length] = new Option( aDict.getValueFor('OBJECTNAME'), id);
		...
		if (!!dObj) {
			...
			if (sel > -1) {
				...
				if (!!oObj) {
					...
					var dragHandle = '<div id="div_dragHandle_' + cObj.options[sel].value + '" onmousedown="handle_onmousedown_beginDrag(this, \'selection_objects2Link\'); return false;" onmouseover="handle_onmouseover_beginDrag(this); return false;" onmouseout="handle_onmouseout_beginDrag(this); return false;">' + 'Object: ' + cObj.options[sel].text + '</div>';
...
function handle_onChange_objects2Link(oSel) {
	...
	if (!!oSel) {
		...
		_alert('oSel.options[' + sel + '].text = [' + oSel.options[sel].text + ']' + ', oSel.options[' + sel + '].value = [' + oSel.options[sel].value + ']');
	}
}
		...
		if (!bool_isAnyErrorRecords) {
			...
			if (!!cObj) {
				...
				if (cObj.innerHTML.length == 0) {
					...
					_html += beginTable('Objects to Link', '&tr=' + 'bgcolor="silver"'.URLEncode() + '&td=' + 'align="center"'.URLEncode() + '&span=' + 'class="boldPromptTextClass"'.URLEncode());
					_html += '<tr>' + '<td>';
