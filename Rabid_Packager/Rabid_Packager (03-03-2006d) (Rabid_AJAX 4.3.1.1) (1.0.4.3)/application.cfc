	...
	<cffunction name="onError">
	   ...
	   <cfscript>
			...
			if (NOT ( (EventName IS "onSessionEnd") OR (EventName IS "onApplicationEnd") ) ) {
				...
				writeOutput('var _parent = ((!!parent._alert) ? "parent." : "");' & Chr(13));
				writeOutput('var jsCode = "_alert(\"An unexpected error occurred.\")";' & Chr(13));
				writeOutput('eval(_parent + jsCode);' & Chr(13));
				writeOutput('jsCode = "_alert(\"Error Event: #EventName#.\")";' & Chr(13));
				writeOutput('eval(_parent + jsCode);' & Chr(13));
				writeOutput('jsCode = "_alert(\"Error details:\")";' & Chr(13));
				writeOutput('eval(_parent + jsCode);' & Chr(13));
				...
				if (IsStruct(Request.commonCode)) {
					for (i = 1; i lte n; i = i + 1) {
						writeOutput('jsCode = "_alert(\"#JSStringFormat(cAR[i])#\")";' & Chr(13));
						writeOutput('eval(_parent + jsCode);' & Chr(13));
					}
				}
