	...
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<!--//
			...
			function doAJAX_func(cmd, callBackFuncName, vararg_params) {
				...
				if (this.isDebugMode()) _alert('argNames.length = [' + argNames.length + ']');
				if (this.isDebugMode()) _alert('argNames = [' + argNames + ']');
				if (this.isDebugMode()) _alert('sValue = [' + sValue + ']');
...
<cfif (Request.commonCode.isServerLocal())>
	...
	<div id="div_contentHome" style="width: 990px; height: 400px;"></div>
