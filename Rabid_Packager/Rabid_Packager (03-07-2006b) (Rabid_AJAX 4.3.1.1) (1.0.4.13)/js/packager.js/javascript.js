function queueUp_AJAX_Sessions(sCmd, sArgsQueryString) {
	...
	switch (sCmd) {
		case js_const_chgDataForObject_symbol:
			...
			doAJAX_func('performSetGeonosisObject', 'displayObjectsBrowser(' + oAJAXEngine.js_global_varName + ')', sArgsQueryString);
		break;
...
function isIdentity(id) {
	...
	var vAR = objectAttributesMetaDataStack[0].getRecordForMatching('COLUMN_NAME', colName, true);
	vAR = ((typeof vAR == const_object_symbol) ? vAR.pop() : vAR);
	var vType = vAR[0].getValueFor('TYPE_NAME');
...
function handleEditableGrid_onClick(cObj, qStringEnc) {
	...
	//	_alert('objectsMetaDataStack = [' + objectsMetaDataStack + ']');
	//	_alert('\nobjectAttributesMetaDataStack = [' + objectAttributesMetaDataStack + ']');
		if ( (!!dObj) && (!!eObj) ) {
...
function displayObjectsBrowserGUI() {
	...
	if ( (!!dObj) && (!!cObj) ) {
		...
		if (sel > -1) {
			...
			var oObj = -1;
	//	_alert('displayObjectsBrowserGUI() :: ' + 'cObj.options[' + sel + '].text = [' + cObj.options[sel].text + ']' + ', aDict = [' + aDict + ']');
