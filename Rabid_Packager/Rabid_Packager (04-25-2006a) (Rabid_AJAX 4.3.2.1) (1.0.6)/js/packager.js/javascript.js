...
function performConfirmDropAttributeDialog() {
	...
	_alert('performConfirmDropAttributeDialog() :: ' + 'cObj = [' + cObj + ']' + ', _cObj = [' + _cObj + ']');
	if (!!cObj) {
	_alert('performConfirmDropAttributeDialog() :: ' + 'cObj.selectedIndex = [' + cObj.selectedIndex + ']' + ', cObj.options.length = [' + cObj.options.length + ']');
	_alert('performConfirmDropAttributeDialog() :: ' + '_cObj.selectedIndex = [' + _cObj.selectedIndex + ']' + ', _cObj.options.length = [' + _cObj.options.length + ']');
	for (var i = 0; i < cObj.options.length; i++) {
		_alert('cObj.options[' + i + '].text = [' + cObj.options[i].text + ']' + ', cObj.options[' + i + '].value = [' + cObj.options[i].value + ']');
	}
	for (var i = 0; i < _cObj.options.length; i++) {
		_alert('_cObj.options[' + i + '].text = [' + _cObj.options[i].text + ']' + ', _cObj.options[' + i + '].value = [' + _cObj.options[i].value + ']');
	}
...
//	_alert('eAR = [' + eAR + ']');
					if ( (eAR.length == 3) || (eAR.length == 4) ) {
						...
						if (eAR.length == 4) {
							...
							if (sel > -1) {
								var datumDict = DictionaryObj.getInstance(qbObj.options[sel].value);
								var colName = qData.getValueFor('UPDATEDCOLUMN');
								var colVal = qData.getValueFor('UPDATEDCOLUMNVALUE');
								datumDict.put(colName, colVal);
//	_alert('colName = [' + colName + ']' + ', colVal = [' + colVal + ']');
//	_alert('datumDict = [' + datumDict + ']');
...
function displayAddOrDropAttributeCompletion(qObj) {
	...
	function dictFromRecord(_ri, _dict, _rowCntName) {
		aDict = DictionaryObj.getInstance(_dict.asQueryString());
	}
			...
			if (sCmd.toUpperCase() == js_const_dropAttribute_symbol.toUpperCase()) {
				if (!!qbObj) {
					...
					for (i = 0; i < qbObj.options.length; i++) {
						qbDict = DictionaryObj.getInstance(qbObj.options[i].value);
