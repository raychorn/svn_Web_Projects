			...
			<cfoutput>#populate_JS_queryObj(Request.qryObj, '_g_.qP', true)# if (!!#Request.___jsName___#) #Request.___jsName___#.init(); 
				<cfif (IsDefined("Request.qryData"))>
					<cfif (IsArray(Request.qryData))>
						...
						#populate_JS_queryObj(qNames, '_g_.qNames', true)# #Request.___jsName___#.p('qDataNames', _g_.qNames); 
						<cfloop index="_i" from="1" to="#n#"><cfset queryName = queryNamesAR[_i]>#populate_JS_queryObj(Request.qryData[_i].qObj, '_g_.qD#_i#', true)# #Request.___jsName___#.p('#queryName#', _g_.qD#_i#);</cfloop>
					</cfif>
				</cfif>
