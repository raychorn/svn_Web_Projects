		...
		function displayMetaDataRecord(_ri, _dict, _rowCntName) {
			...
			if (_TYPE_NAME.toUpperCase().indexOf(' identity'.toUpperCase()) == -1) {
				...
				if (bool_justRefresh) {
					...
					html += '<td><input id="' + idName + '" class="normalStatusClass" size="' + ((_COLUMN_SIZE > 50) ? 50 : _COLUMN_SIZE) + '" maxlength="' + _COLUMN_SIZE + '" onkeydown="if (event.keyCode == 13) { processAddClassInputAction(this.id); }; return true;"></td>';
					html += '</tr>';
				}
