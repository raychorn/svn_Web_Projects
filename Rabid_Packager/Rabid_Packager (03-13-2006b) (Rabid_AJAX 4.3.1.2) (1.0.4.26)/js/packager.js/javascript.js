...
function queueUp_AJAX_Sessions(sCmd, sArgsQueryString) {
	...
	switch (sCmd) {
		case js_const_addClassDef_symbol:
			...
			doAJAX_func('performAddGeonosisClassDef', 'displayAddClassDefCompletion(' + oAJAXEngine.js_global_varName + ')', sArgsQueryString);
		break;
...
function processAddClassInputAction(id, qStr) {
	...
	//	_alert('processAddClassInputAction(id = [' + id + '], qStr = [' + unescape(qStr) + '])');
...
function displayClassesBrowser(qObj) {
	...
	function displayClassBrowser(qO, nR) {
		...
		if (!!qData) {
			...
			_html += beginTable('Add a Class', '&tr=' + 'bgcolor="silver"'.URLEncode() + '&td=' + 'align="center"'.URLEncode() + '&span=' + 'class="boldPromptTextClass"'.URLEncode());
			_html += '<tr>' + '<td>';
			_html += beginTable();
...
function displayAddClassDefCompletion(qObj) {
	var qData = -1;
	var nRecs = -1;
	var oParms = -1;
	var _contextName = js_const_Objects_symbol;
