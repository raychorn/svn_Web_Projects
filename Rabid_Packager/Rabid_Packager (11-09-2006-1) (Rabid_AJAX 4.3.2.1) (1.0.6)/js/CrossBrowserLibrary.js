...
function isServerLocal() {
	return (document.location.href.toUpperCase().indexOf('laptop.halsmalltalker.com'.toUpperCase()) != -1);
}
...
function handle_ToolTip_MouseOver(ev, id, parked_id) {
	...
	if (!!obj) {
		...
		if ( (!!parked_id) && (parked_id.trim().length > 0) ) {
		...
		if (!!tt_obj) {
			var _clientHeight = clientHt$();
			var _clientWidth = clientWid$();
...
function flushCache$(oO) {
	if ( (!!oO) && (!!oO.innerHTML) && (oO.innerHTML.length > 0) ) {
...
function $(id) {
	var obj = null;
	if (typeof id == const_string_symbol) {
		if ( ($cache[id] == null) || ($cache[id].nodeName == 'SELECT') ) {
			obj = _$(id);
			$cache[id] = obj;
		} else {
			obj = $cache[id];
...
function _fromHex(ch) {
	var HEX = "0123456789ABCDEF";
	if (!!ch) {
		return ((HEX.indexOf(ch.charAt(0))) << 4) + HEX.indexOf(ch.charAt(1));
	}
	return ch;
}
...
function _fromHex_() {
	return _fromHex(this);
}

String.prototype.fromHex = _fromHex_;
...
//	_alert('ar[' + i + '].trim().length = [' + ar[i].trim().length + ']' + ', ar[' + i + '].trim() = [' + ar[i].trim() + ']' + ', [' + ar[i].trim().explainChars() + ']');
...
function clipCaselessReplace(keyword, sText, bool_All) {
	bool_All = ((bool_All == true) ? bool_All : false);
	while (true) {
...
function _setAbstractScopeDebugPanelContent(divName, _html) {
	var dbObj = $(divName);
	if (!!dbObj) {
		dbObj.style.width = ((clientWid$() - 25) / 4) + 'px';
