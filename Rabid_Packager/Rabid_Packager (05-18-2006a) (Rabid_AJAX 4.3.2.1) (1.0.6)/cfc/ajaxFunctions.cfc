					...
					switch (Request.qryStruct.cfm) {
						case 'performCheckGeonosisLinkedObjects':
							if ( (IsDefined("Request.qryStruct.OBJECTID1")) AND (IsDefined("Request.qryStruct.OBJECTID2")) ) {
								aGeonosisLinkedObjectsCollector1 = Request.commonCode.objectForType('geonosisObjectLinksCollector').init(Request.Geonosis_DSN).getObjectLinksForObjectId(Request.commonCode.filterIntForSQL(Request.qryStruct.OBJECTID1));

								if (NOT Request.dbError) {
									hasLinks = ( (IsQuery(aGeonosisLinkedObjectsCollector1.qGetObjectLinks)) AND (aGeonosisLinkedObjectsCollector1.qGetObjectLinks.recordCount gt 0) );

									aGeonosisLinkedObjectsCollector2 = Request.commonCode.objectForType('geonosisObjectLinksCollector').init(Request.Geonosis_DSN).getObjectLinksForObjectId(Request.commonCode.filterIntForSQL(Request.qryStruct.OBJECTID2));
									
									if (NOT Request.dbError) {
										hasLinks = ( (hasLinks) OR ( (IsQuery(aGeonosisLinkedObjectsCollector2.qGetObjectLinks)) AND (aGeonosisLinkedObjectsCollector2.qGetObjectLinks.recordCount gt 0) ) );
