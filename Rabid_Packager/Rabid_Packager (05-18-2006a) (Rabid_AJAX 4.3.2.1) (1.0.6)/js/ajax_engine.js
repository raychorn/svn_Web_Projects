...
AJAXEngine.prototype = {
	...
	create : function(divName) {  // // define create(); prototype
		...
		this.height = ((this.isReleaseMode()) ? "1" : "415") + 'px';
	...
	sendPacket : function(packet, _bUseCache) {  // define sendPacket(); prototype
		...
		this.sent = '&___jsName___=' + this.js_global_varName + '&enkrip2=' + ((this.useEnkrip2) ? '1' : '0') + packet;

		var js = 'AJAXEngine.serializeAndSend(' + String(bUseCache) + ', AJAXEngine.$[' + this.id + '])';
	...
	formatPacket : function(packet) {
		if( this.isWddxEnabled() ){
			...
			if (typeof packet == "string") return packet;
				...
				var p = [];
				for( var k in packet ) p[p.length] = k + "=" + URLEncode(URLDecode(packet[k]));  // this may look funny but it allows for the packet to be either URLEncoded or not with no errors either way...
				return "&" + p.join("&");
			}
		}
	},
	_serializeAndSend : function(_bUseCache) {  // define sendPacket(); prototype
		...
		if (this.isCurrentContextValid()) {
			...
			var aDict = DictionaryObj.get$(this.sent);
			...
			this.sent += tDict.asQueryString(); // DictonaryObj needs to encode the parms AFTER decoding them...

			DictionaryObj.remove$(aDict.id);
			if (isContextShifted) DictionaryObj.remove$(tDict.id);
	...
	addNamedContext : function(aName, parmsQueryString) {
		...
		if (!this.namedContextCache[aName]) {
			aDict = DictionaryObj.get$(parmsQueryString);
			oDict = DictionaryObj.get$();
			pDict = DictionaryObj.get$();
			argCnt = aDict.length();
			keys = aDict.getKeys();
