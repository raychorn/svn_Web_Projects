	...
	if (!!cObj) {
		...
		if (!!dObj) {
			...
			if (sel > -1) {
				...
				queueUp_AJAX_Sessions(js_const_getOBJECTS_symbol, '&OBJECTCLASSID=' + cObj.options[sel].value.toString().URLEncode());
...
//	_alert('==============================================================\n\n');
	...
	if (nRecs > 0) {
		...
		if (0) {
			var _METADATACNT = 4;  // assume there are 4 metadata records to deal with... by default...
			
			// BEGIN: Is there a schema query that contains the METADATACNT entity ?  If so, use it and adjust the processing to skip over the metadata queries...
			qData = qObj.named('qData' + 1);
		//	_alert('+++ qData = [' + qData + ']');
		//	_alert('==============================================================\n\n');
		//	_alert('(!!qData) = [' + (!!qData) + ']');
			if (!!qData) {
				try {
					_METADATACNT = qData.getValueFor('METADATACNT');
					if (typeof _METADATACNT == const_object_symbol) {
						_METADATACNT = parseInt(_METADATACNT.pop());
					}
				} catch(ee) {
					_alert(jsErrorExplainer(ee, '111'));
				} finally {
				}
				try {
				//	_alert('*** _METADATACNT.length = [' + _METADATACNT.length + ']' + ', (typeof _METADATACNT) = [' + (typeof _METADATACNT) + ']' + ', _METADATACNT = [' + _METADATACNT + ']');
				//	_alert('==============================================================\n\n');
				} catch(ee) {
					_alert(jsErrorExplainer(ee, '222'));
				} finally {
				}
			}
			// END! Is there a schema query that contains the METADATACNT entity ?  If so, use it and adjust the processing to skip over the metadata queries...
			...
			for (i = (_METADATACNT + 1); i <= (nRecs - 1); i += 2) {
