...
DragObj.$ = [];
DragObj.get$ = function(el, constraint) {
	if(!el.id) {
		el.id = "DragObj" + DragObj.$.length;
	}

	var instance = DragObj.$[el.id];
	if(instance == null) {
		instance = DragObj.$[el.id] = DragObj.$[DragObj.$.length] = new DragObj(el, constraint);
	}
	return instance;
};
...
DropTarget.get$ = function(el) {
	var instance = DropTarget.$[el.id];
	if(instance == null) {
		instance = DropTarget.$[el.id] = new DropTarget(el);
	}
	return instance;
}
