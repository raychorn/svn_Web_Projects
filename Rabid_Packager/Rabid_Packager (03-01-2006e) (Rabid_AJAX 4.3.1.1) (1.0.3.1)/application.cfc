	...
	<cffunction name="onError">
	   ...
	   <cfscript>
			...
			if (NOT ( (EventName IS "onSessionEnd") OR (EventName IS "onApplicationEnd") ) ) {
				...
				writeOutput('_alert("An unexpected error occurred.");' & Chr(13));
				writeOutput('_alert("Error Event: #EventName#");' & Chr(13));
				writeOutput('_alert("Error details:");' & Chr(13));
				...
				if (IsStruct(Request.commonCode)) {
					for (i = 1; i lte n; i = i + 1) {
						writeOutput('_alert("#JSStringFormat(cAR[i])#");' & Chr(13));	
					}
				}
