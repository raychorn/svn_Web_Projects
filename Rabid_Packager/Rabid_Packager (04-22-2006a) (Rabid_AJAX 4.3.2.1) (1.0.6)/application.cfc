	   ...
	   <cfscript>
			...
			if (NOT ( (EventName IS "onSessionEnd") OR (EventName IS "onApplicationEnd") ) ) {
				...
				if (NOT bool_is_cfcontent_js) {
					...
					writeOutput('var _reportError = ((!!parent._alertError) ? "parent._alertError" : "_alertError");' & Chr(13));
					writeOutput('var jsCode = "_reportError(\"An unexpected error occurred.\")";' & Chr(13));
					writeOutput('_reportError = ((!!_reportError) ? _reportError : alert);' & Chr(13));
					writeOutput('if (_reportError == "alert") { _parent = ""; };' & Chr(13));
					writeOutput('eval(_parent + jsCode);' & Chr(13));
					writeOutput('jsCode = "_reportError(\"Error Event: #EventName#.\")";' & Chr(13));
					writeOutput('eval(_parent + jsCode);' & Chr(13));
					writeOutput('jsCode = "_reportError(\"Error details:\")";' & Chr(13));
					writeOutput('eval(_parent + jsCode);' & Chr(13));
					...
					if (IsStruct(Request.commonCode)) {
						for (i = 1; i lte n; i = i + 1) {
							writeOutput('jsCode = "_reportError(\"#JSStringFormat(cAR[i])#\")";' & Chr(13));
							writeOutput('eval(_parent + jsCode);' & Chr(13));
						}
					}
					...
					writeOutput('_db += "Error Event: #EventName#." + "\n";' & Chr(13));
