...
function queueUp_AJAX_Sessions(sCmd, sArgsQueryString) {
	...
	switch (sCmd) {
		case js_const_chgDataForObject_symbol:
			...
			doAJAX_func('performSetGeonosisObject', 'displayObjectsBrowserEditCompletion(' + oAJAXEngine.js_global_varName + ')', sArgsQueryString);
		//	doAJAX_func('performSetGeonosisObject', '', sArgsQueryString); // do nothing upon return because all the data is already loaded in the client...
		break;
...
function displayObjectsBrowserEditCompletion(qObj) {
	...
	if (nRecs > 0) {
		...
		qData.iterateRecObjs(anyErrorRecords);
		if (!bool_isAnyErrorRecords) {
			_alert('stack_breadCrumbs[stack_breadCrumbs.length - 1] = [' + stack_breadCrumbs[stack_breadCrumbs.length - 1] + ']');
		} else {
			qData.iterateRecObjs(displayErrorRecord);
		}
	}
