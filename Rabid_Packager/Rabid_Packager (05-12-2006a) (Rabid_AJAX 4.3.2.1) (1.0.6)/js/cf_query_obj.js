...
QueryObj.i$ = function(ar) {
	...
	try {
		...
		//	_alert('A. ar[0] = [' + ar[0] + ']');
			...
			_alert('A.1 oQuery = [' + oQuery + ']' + ', oQuery.id = [' + oQuery.id + ']');
			colAR = ar[0].split(',');
		//	_alert('B. colAR = [' + colAR + ']');
		//	_alert('C. ar.length = [' + ar.length + ']' + ', colAR.length = [' + colAR.length + ']');
			for (i = 1; i < ar.length; i += 2) {
				_alert('D. ar[' + i + '] = [' + ar[i] + ']' + ', ar[' + (i + 1) + '] = [' + ar[i + 1] + ']' + ', row = [' + row + ']');
				oQuery.qsc$(ar[i],ar[i + 1],row);
				row++;
			}
			_alert('\nZ. oQuery = [' + oQuery + ']' + ', oQuery.id = [' + oQuery.id + ']' + ', oQuery.dataRec.length = [' + oQuery.dataRec.length + ']' + ', oQuery.dataRec = [' + oQuery.dataRec + ']' + '\n');
...
QueryObj.prototype = {
	...
	QuerySetCells : function(cName, vValAR, rowNum) {
		...
		var ci = -1;
		var ri = -1;
		_alert('1. (typeof vValAR) = [' + (typeof vValAR) + ']');
		if (typeof vValAR != const_object_symbol) {
			var ar = [];
			ar.push(vValAR);
			vValAR = ar;
		}
		_alert('2. (typeof vValAR) = [' + (typeof vValAR) + ']');
		rowNum = ((rowNum == null) ? 1 : rowNum);
		_alert('3. rowNum = [' + rowNum + ']');
		var colNum = this.getColNumFromColName(cName);
		_alert('4. colNum = [' + colNum + ']');
		if (colNum != -1) {
			for (ri = 0; ri < vValAR.length; ri++) {
				d = this.dataRec[rowNum + ri];
		_alert('5. (typeof d) = [' + (typeof d) + ']' + ', d = [' + d + ']' + ', (d == null) = [' + (d == null) + ']');
				if ( (d == null) || (typeof d != const_object_symbol) ) {
					d = [];
				}
		_alert('6. (typeof d) = [' + (typeof d) + ']' + ', d = [' + d + ']' + ', (d == null) = [' + (d == null) + ']');
				d[colNum] = vValAR[ri].URLDecode();
				this.dataRec[rowNum + ri] = d;
			}
		_alert('7. this.dataRec.length = [' + this.dataRec.length + ']');
		}
		return false;
	},
