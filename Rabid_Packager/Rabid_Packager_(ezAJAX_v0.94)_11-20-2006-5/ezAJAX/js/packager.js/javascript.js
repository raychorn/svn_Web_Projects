...
function handle_className_onChange(cObj) {
	...
	if ( (!!cObj) && (!!dObj) && (!!oObj) ) { // cObj.options[sel].value is the OBJECTCLASSID
		...
		if (sel > -1) {
			...
			oAJAXEngine.doAJAX('performGetGeonosisObjects', 'displayObjectsBrowser', 'CLASSNAME', cObj.options[sel].text.toString().ezURLEncode(), 'hasMetaData', (((objectsMetaDataStack.length > 0) && (objectAttributesMetaDataStack.length > 0)) ? 'true' : 'false').ezURLEncode());
		}
	}
}
...
function displayObjectsBrowser(qObj) {
	...
	ezAlert('displayObjectsBrowser :: qObj = [' + qObj + ']');
