					...
					case 'performGetGeonosisObjects':
						if (Request.qryStruct.argCnt gte 1) {
							if (IsDefined("Request.qryStruct.CLASSNAME")) {
								...
								QuerySetCell(qSchema, 'scopesContentQuery', (aGeonosisObjCollector.collection.count * 2) + 2 + 1 + 1, qSchema.recordCount);
								...
								execViaSessionLock('Session.metadataStruct = StructNew(); Session.metadataStruct.bool_objectsMetData = true; Session.metadataStruct.bool_objectAttributesMetData = true;');
							...
							//	Request.commonCode.cf_dump(qContent, 'qContent', false);
