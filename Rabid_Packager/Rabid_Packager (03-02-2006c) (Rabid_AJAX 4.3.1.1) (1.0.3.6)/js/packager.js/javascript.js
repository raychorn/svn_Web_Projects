		...
		if (!!cObj) {
			...
			if (bcAR.length > 0) {
				...
				for (i = 0; i < bcAR.length; i++) {
					_ar = bcAR[i].split('|');
					if (_ar.length > 1) {
						// _ar contains an array with the first item being the function name, the second item is the id of the DOM object and the third item (if any) is the parm to add to the DOM object when calling the function...
					//	_alert('bcAR[' + i + '] = [' + bcAR[i] + ']');
					//	_alert('_ar.length = [' + _ar.length + ']' + ', _ar = [' + _ar + ']');
