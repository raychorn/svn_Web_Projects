...
GeonosisObj.searchInstancesForObjectID = function(anID) {
	...
	function searchRecord(_ri, _dict, _rowCntName) {
		...
		try {
			_OBJECTID = _dict.getValueFor('OBJECTID');
			_rowCnt = _dict.getValueFor(_rowCntName);
		} catch(e) {
		} finally {
		}
	...
	var i = -1;
	for (i = 0; i < GeonosisObj.instances.length; i++) {
		GeonosisObj.instances[i].qAttrs.iterateRecObjs(anyErrorRecords);
		if (!bool_isAnyErrorRecords) {
			GeonosisObj.instances[i].qAttrs.iterateRecObjs(searchRecord);
		}
	}
