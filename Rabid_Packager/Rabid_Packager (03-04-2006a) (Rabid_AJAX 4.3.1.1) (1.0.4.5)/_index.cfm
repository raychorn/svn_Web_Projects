...
<cfif (Request.commonCode.isServerLocal())>
	...
	<div id="div_ajaxSystem" style="position: absolute;"></div>
		...
		<!--
			...
			var dbObj = $('div_application_debug_panel');
			if (!!dbObj) {
			//	dbObj.style.top = '100px';
			//	dbObj.style.left = '100px';
				dbObj.style.width = (clientWidth() - 100) + 'px';
				dbObj.style.height = '25px';
				_html = '';
				<cfscript>
					ar = ListToArray(content_application_debug_panel, Chr(13));
					n = ArrayLen(ar);
					styleContent = '';
					jsContent = '';
					...
					for (i = 1; i lte n; i = i + 1) {
						...
						if (bool_insideStyleTags) {
							...
							if (bool_insideStyleTags) {
								styleContent = styleContent & t & Chr(13);
								t = '';  // force the end of style tag to be removed from the data stream...
							}
						} else {
							if (FindNoCase('<style>', t) gt 0) {
