...
function displayClassesBrowser(qObj) {
	...
	function displayClassBrowser(qO, nR) {
		...
		if (!!qData) {
			...
			_html += beginTable('Add Classes', '&tr=' + 'bgcolor="silver"'.URLEncode() + '&td=' + 'align="center"'.URLEncode() + '&span=' + 'class="boldPromptTextClass"'.URLEncode());
			_html += '<tr>' + '<td>';
_alert('objectClassDefinitionsMetDataStack = [' + objectClassDefinitionsMetDataStack + ']');
			_html += '</td>' + '</tr>';
			_html += endTable();
		}
		return _html;
	...
	if (nRecs > 0) {
		...
		if (!bool_isAnyErrorRecords) {
			...
			if (!!cObj) {
				...
				_html += displayClassBrowser(qObj, 1); // nRecs
				_html += '</td>';
