<cfcomponent displayname="ajaxCode" output="Yes" extends="ezClusterCode">
	...
	<cffunction name="js_jsCode_" access="public" returntype="string">
		...
		<cfset var qStats = -1>
		<cfset var queryName = "">
		<cfset var delayedErrorMsgs = "">
		<cfset var queryNamesAR = ArrayNew(1)>
		<cfset var qFunc = populate_JS_queryObj>

		<cfsavecontent variable="Request._jsCode_">
			<cfoutput>#qFunc(Request.qryObj, '_g_.qP', true)# if (!!#Request.___jsName___#) #Request.___jsName___#.init(); 
				<cfif (Request.jsQueryObjError)><cfset delayedErrorMsgs = ListAppend(delayedErrorMsgs, Request.jsQueryObjErrorMsg, ",")></cfif>
				<cfif (IsDefined("Request.qryData"))>
					<cfif (IsArray(Request.qryData))>
						...
						#qFunc(qStats, '_g_.qStats', true)# #Request.___jsName___#.p('qDataNum', _g_.qStats); 
						<cfif (Request.jsQueryObjError)><cfset delayedErrorMsgs = ListAppend(delayedErrorMsgs, Request.jsQueryObjErrorMsg, ",")></cfif>
						<cfscript>
							qNames = QueryNew('id, name'); 
						</cfscript>
						<cfloop index="_i" from="1" to="#n#">
							<cfset queryName = "qData#_i#">
							<cfif (Len(Request.qryData[_i].aName) gt 0)>
								<cfset queryName = Request.qryData[_i].aName>
							</cfif>
							<cfset queryNamesAR[ArrayLen(queryNamesAR) + 1] = queryName>
							<cfscript>
								QueryAddRow(qNames, 1); 
								QuerySetCell(qNames, 'id', qNames.recordCount, qNames.recordCount); 
								QuerySetCell(qNames, 'name', queryName, qNames.recordCount); 
							</cfscript>
						</cfloop>
						#qFunc(qNames, '_g_.qNames', true)# #Request.___jsName___#.p('qDataNames', _g_.qNames); 
						<cfif (Request.jsQueryObjError)><cfset delayedErrorMsgs = ListAppend(delayedErrorMsgs, Request.jsQueryObjErrorMsg, ",")></cfif>
						<cfloop index="_i" from="1" to="#n#"><cfset queryName = queryNamesAR[_i]>#qFunc(Request.qryData[_i].qObj, '_g_.qD#_i#', true)# #Request.___jsName___#.p('#queryName#', _g_.qD#_i#);<cfif (Request.jsQueryObjError)><cfset delayedErrorMsgs = ListAppend(delayedErrorMsgs, Request.jsQueryObjErrorMsg, ",")></cfif></cfloop>
					</cfif>
					<cfif (Len(delayedErrorMsgs) gt 0)>
						<cfscript>
							qObj = QueryNew('id, errorMsg, moreErrorMsg, explainError, isPKviolation');
							QueryAddRow(qObj, 1);
							QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
							QuerySetCell(qObj, 'errorMsg', delayedErrorMsgs, qObj.recordCount);
						...
						<cfset queryName = "qERROR">#qFunc(qObj, '_g_.qD#_i+1#', true)# #Request.___jsName___#.p('#queryName#', _g_.qD#_i+1#);
					</cfif>
				</cfif>
	...
	<cfscript>
		...
		function registerNamedQueryFromAJAX(aName, qObj) {
			...
			if (NOT IsDefined("Request.qryData")) {
				...
				Request.qryData[ArrayLen(Request.qryData) + 1] = aStruct;
			} else {
		...
		function registerQueryFromAJAX(qObj) {
			return registerNamedQueryFromAJAX('', qObj);
		}
		...
		function endAJAX() {
			...
			if (Request.bool_using_xmlHttpRequest) {
				...
				if (Request.qryStruct.enkrip2 eq 1) {
					aStruct = StructNew();
					...
					_jsCode = _jsCode & "try { eval(_xx_) } catch(ex) { _alertError(jsErrorExplainer(ex, 'via xmlHttpRequest :: #CGI.SCRIPT_NAME# :: endAJAX()')); \/* _alert(_xx_); *\/ } finally {};";
				}
			...
			//	writeOutput('<b>_jsCode = [#_jsCode#]</b><br>');
				...
				if (Request.qryStruct.enkrip2 eq 1) {
					...
					_jsCode = aStruct.CIPHERTEXT;
					_jsCode = _jsCode & aStruct.DECIPHER;
					_jsCode = _jsCode & "try { eval(_xx_) } catch(ex) { _alertError(jsErrorExplainer(ex, 'via iframe :: #CGI.SCRIPT_NAME# :: endAJAX()')); \/* _alert(_xx_); *\/ } finally {};";
				}
