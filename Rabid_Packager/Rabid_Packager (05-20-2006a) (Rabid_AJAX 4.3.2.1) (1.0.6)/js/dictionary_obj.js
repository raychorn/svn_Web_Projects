...
DictionaryObj.get$ = function(aSpec) {
	...
	var instance = DictionaryObj.$[DictionaryObj.$.length];
	if(instance == null) {
		instance = DictionaryObj.$[DictionaryObj.$.length] = new DictionaryObj(DictionaryObj.$.length);
	}
	instance.fromSpec(aSpec);
	return instance;
};

DictionaryObj.remove$ = function(id) {
	var ret_val = false;
	if ( (id > -1) && (id < DictionaryObj.$.length) ) {
		var instance = DictionaryObj.$[id];
		if (!!instance) {
			DictionaryObj.$[id] = object_destructor(instance);
			ret_val = (DictionaryObj.$[id] == null);
		}
	}
	return ret_val;
};

DictionaryObj._remove$ = function() {
	var ret_val = true;
	for (var i = 0; i < DictionaryObj.$.length; i++) {
		DictionaryObj.remove$(i);
	}
	DictionaryObj.$ = [];
	return ret_val;
};
...
DictionaryObj.prototype = {
	...
	dictForColumn : function(colNum) { // this method returns a dict obj for a specific column of databased on the way the getValueFor method returns data...
		return DictionaryObj.get$(this.asQueryStringForColumn(colNum));
	},
	...
	destructor : function() {
		return (this.id = DictionaryObj.$[this.id] = this.keys = this.cache = null);
	},
	dummy : function() {
		return false;
	}
