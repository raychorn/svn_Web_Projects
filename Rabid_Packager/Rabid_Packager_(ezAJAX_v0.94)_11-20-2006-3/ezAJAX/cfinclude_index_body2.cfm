...
<cfoutput>
	...
	ezAlert('handleSampleAJAXCommand :: qObj = [' + qObj + ']');
		...
		function handle_ezAJAXCallBack(qObj, nRecs, qStats, argsDict, qData1) {
			var aDict = -1;
			...
			ezAlert('handle_ezAJAXCallBack :: nRecs = [' + nRecs + ']');

			if (!!qStats) {
				ezAlert('handle_ezAJAXCallBack :: qStats = [' + qStats + ']');

				if (!!argsDict) {
					ezAlert('handle_ezAJAXCallBack :: argsDict = [' + argsDict + ']');

					if (!!qData1) {
						qData1[0].iterateRecObjs(searchForStatusRecs);
						ezAlert('handle_ezAJAXCallBack :: qData1 = [' + qData1 + ']');
					} else {
