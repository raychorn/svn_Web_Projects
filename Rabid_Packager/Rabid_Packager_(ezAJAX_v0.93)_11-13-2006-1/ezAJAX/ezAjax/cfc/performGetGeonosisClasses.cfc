	...
	<cfscript>
		function _userDefinedAJAXFunctions(qryStruct) {
			...
			try {
				...
				QueryAddRow(qObj, 1);
				QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
				QuerySetCell(qObj, 'errorMsg', '<font color="red"><b>Something just went wrong in "#CGI.SCRIPT_NAME#"... :: Reason: "#errMsg#".</b></font><br>' & ezCfDump(Request.commonCode, 'Request.commonCode', true), qObj.recordCount);
