	...
	<cffunction name="sql_GeonosisAddObject" access="private" returntype="string">
		...
		<cfscript>
			...
			for (i = 1; i lte n; i = i + 1) {
				...
				try {
			writeOutput('<b>A. (+++) colsList[#i#] = [#colsList[i]#], _dataStruct_[colsList[#i#]] = [#_dataStruct_[colsList[i]]#]</b><br>');
					colVal = URLDecode(_dataStruct_[colsList[i]]);
			writeOutput('<b>B. (+++) colVal = [#colVal#]</b><br>');
				} catch (Any e) {
					bool_dataElementExists = false;
				}
			writeOutput('<b>C. (+++) bool_dataElementExists = [#bool_dataElementExists#]</b><br>');
