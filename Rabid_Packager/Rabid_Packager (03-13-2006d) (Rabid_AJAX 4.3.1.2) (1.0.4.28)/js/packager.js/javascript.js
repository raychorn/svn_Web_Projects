...
function performConfirmDropObjectDialog() {
	...
	if (!!cObj) {
		if ((sel = cObj.selectedIndex) > -1) {
			...
			if (xBool) {
			//	queueUp_AJAX_Sessions(js_const_dropClassDef_symbol, '&objectID=' + objectID.URLEncode());
			}
		}
	}
}
...
function displayObjectsBrowserGUI() {
	...
	if ( (!!dObj) && (!!cObj) ) {
		...
		if (sel > -1) {
			...
			var _btnHtmlL = '<button id="btn_objects_remove" disabled class="buttonClass" title="Click this button to remove the selected Object." onclick="performConfirmDropObjectDialog(); return false;">[-]</button>&nbsp;';
