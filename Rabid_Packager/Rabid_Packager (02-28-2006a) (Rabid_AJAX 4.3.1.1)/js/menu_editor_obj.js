...
menuEditorObj.processClipboardPasteBecomesMove = function(anObj) {
	...
	var pmObj = $(menuEditorObj.const_clipboard_paste_to_menu);
...
menuEditorObj.defaultGUI = function(menuEditorObj, _site_menu_text_color, _outerTableWidth) {
	...
	_html += '<input type="button" id="addit_menu_item" value="Add Menu Item" class="buttonClass" title="Click this button to add a menu item.  Menu items are used to link to internal pages of content or to external documents such as Panagons docs or to external sites.  Menu items are different than SubMenu Items in that a Menu Item provides a single link to something whereas a SubMenu provides a container for a number of Menu Items." onClick="var brObj = $(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuItemAdditor(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '<tr>';
	...
	_html += '<input type="button" id="menu_browse_into" value="Open\nSub Menu" disabled class="buttonClass" title="Click this button to Open a SubMenu, the SubMenu that is highlighted in the list directly above this button.  The other way to Open a SubMenu is to double-click the SubMenu item." onClick="var brObj = $(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuBrowseInto(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '<td width="50%" align="right">';
	_html += '<input type="button" id="menu_browse_out" id="menu_browse_out" value="Close\nSub Menu" disabled class="buttonClass" title="Click this button to Close a SubMenu that was previously opened." onClick="var brObj = $(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuBrowseOut(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '</tr>';
	_html += '</table>';
	_html += '</td>';
	...
	_html += '<br><br>';
	_html += '<input type="button" id="menu_delete_item" disabled value="Delete\nItem" class="buttonClass" title="Click this button to delete a Menu Item from the Menu.  You may also delete a SubMenu after the SubMenu is empty.  Use this button to delete each Menu Item from the SubMenu and then close the SubMenu to use this button to delete the empty SubMenu." onClick="var brObj = $(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuCutToThinAir(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '</tr>';
	_html += '</table>';
	_html += '</td>';
...
//	const_menu_copy_to_clipboard_symbol : menuEditorObj._const_menu_copy_to_clipboard_symbol, // this allows the copy to clipboard to be replaced by a delete item button without having to recode a lot of logic...
	...
	_processMenuCutToClipboard : function(brObj, bool_copy_to_clipboard) {
		...
		var bcpasteObj = $(menuEditorObj.const_clipboard_paste_to_menu);
	...
	_processMenuItemEditor : function(brObj) {
		...
		if ( (!!brObj) && (!!ebObj) && (!!abObj) && (!!asObj) && (!!edObj) && (!!epObj) && (!!clipObj) && (!!pgObj) && (!!hObj) ) {
			...
			if (!!c) {
				...
				var b4Obj = $(menuEditorObj.const_clipboard_paste_to_menu);
	...
	initMenuEditorClipboardObjects : function() {
		...
		if (!!rObj) {
			...
			html += '<td>';
			html += '<center>';
			html += '<select id="menu_clipboard" size="5" style="font-size: 10px; line-height: 10px;" onchange="var clipObj = $(\'menu_clipboard\'); if (!!clipObj) { menuEditorObj.processSelectedClipboardItem(clipObj, menuEditorObj.instances[' + this.id + ']) } return false;">';
			html += '</select>';
			html += '</center>';
			html += '</td>';
			html += '</tr>';
			...
			html += '<tr>';
			html += '<td>';
			html += '<input type="button" id="clipboard_remove_item" value="Remove Clipboard Item" disabled class="buttonClass" onClick="var clipObj = $(\'menu_clipboard\'); if (!!clipObj) { menuEditorObj.processClipboardRemoveSelected(clipObj, menuEditorObj.instances[' + this.id + ']) } return false;">';
			html += '</td>';
			html += '</tr>';
