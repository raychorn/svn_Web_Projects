function jsErrorExplainer(e, funcName, bool_useAlert) {var _db = ''; var msg = '';	bool_useAlert = ((bool_useAlert == true) ? bool_useAlert : false);_db += "e.number is: " + (e.number & 0xFFFF) + '\n'; _db += "e.description is: " + e.description + '\n'; _db += "e.name is: " + e.name + '\n'; _db += "e.message is: " + e.message + '\n';msg = funcName + '\n' + e.toString() + '\n' + _db;if (bool_useAlert) alert(msg);return msg;}
...
function __callback(h) {
	...
	if ( (!!h) && (typeof h == 'string') && (!!h.length) && (h.length > 0) ) {
		...
		//	if (!!_alert) _alert(hh);
			...
			if (!!_alertError) {
				_alertError(jsErrorExplainer(e, '__callback()'));
			} else {
				_alert(jsErrorExplainer(e, '__callback()'));
			}
		} finally {
		}
