...
function processAddAttributeInputAction(id, qStr) {
	...
	var qString = '';
	var bool_hasAllValues = false;
_alert('stack_addAttribute_inEdit = [' + stack_addAttribute_inEdit + ']');
	for (i = 0; i < stack_addObject_inEdit.length; i++) {
		iObj = $(stack_addObject_inEdit[i]);
		if ( (!!iObj) && (iObj.value.trim().length > 0) ) {
			kValuesCnt++;
	...
	if (kValuesCnt == stack_addObject_inEdit.length) {
		...
		var _qStr = '&objectClassID=' + objectClassID + '&CLASSNAME=' + _CLASSNAME + '&hasMetaData=' + (((objectsMetaDataStack.length > 0) && (objectAttributesMetaDataStack.length > 0)) ? 'true' : 'false').URLEncode() + qString;
	//	queueUp_AJAX_Sessions(js_const_addObject_symbol, _qStr);
		_alert(_qStr);
	} else if (currentNum > -1) {
		if (currentNum == (stack_addObject_inEdit.length - 1)) {
			currentNum = 0;
...
function displayObjectsBrowserGUI() {
	...
	if ( (!!dObj) && (!!cObj) ) {
		...
		if (sel > -1) {
			...
			_html += beginTable('Add an Attribute', '&bool=' + 'includeDismissButton=true'.URLEncode() + '&div=' + 'dismissDiv=div_addAttr_panel'.URLEncode() + '&tr=' + 'bgcolor="silver"'.URLEncode() + '&td=' + 'align="center"'.URLEncode() + '&span=' + 'class="boldPromptTextClass"'.URLEncode());
			_html += '<tr>' + '<td>';
			_html += beginTable();
