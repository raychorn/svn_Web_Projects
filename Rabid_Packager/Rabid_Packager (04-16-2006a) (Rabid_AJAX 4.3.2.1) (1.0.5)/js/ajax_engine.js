...
//_alert('\n(sendPacket) :: ' + 'this.sent = [' + this.sent + ']');
	...
	formatPacket : function(packet) {
		if( this.isWddxEnabled() ){
			...
			if (typeof packet == "string") return packet;
				...
				var p = [];
				for( var k in packet ) p[p.length] = k + "=" + escape(unescape(packet[k]));  // this may look funny but it allows for the packet to be either URLEncoded or not with no errors either way...
				return "&" + p.join("&");
			}
		}
	},
...
//_alert('\n1. (_serializeAndSend) :: ' + 'this.sent = [' + this.sent + ']');
			this.sent = aDict.asQueryString();
//_alert('\n2. (_serializeAndSend) :: ' + 'this.sent = [' + this.sent + ']');
//_alert('\n2.1. (_serializeAndSend) :: ' + 'aDict = [' + aDict + ']');
			argCnt = apparentArgCnt;
			this.sent += tDict.asQueryString(); // DictonaryObj needs to encode the parms AFTER decoding them...
//_alert('\n3. (_serializeAndSend) :: ' + 'this.sent = [' + this.sent + ']');
//_alert('\n3.1. (_serializeAndSend) :: ' + 'tDict = [' + tDict + ']');
...
//_alert('\n(_serializeAndSend) :: ' + 'this.sent = [' + this.sent + ']');
