...
function queueUp_AJAX_Sessions(sCmd, sArgsQueryString) {
	...
	switch (sCmd) {
		case js_const_checkLinkedObjects_symbol:
			...
			oAJAXEngine.doAJAX('performCheckGeonosisLinkedObjects', 'displayCheckedLinkedObjectsCompletion', 'sCmd=', sCmd);
		break;
		
		case js_const_getLinkedObjects_symbol:
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performGetGeonosisLinkedObjects', 'displayLinkedObjectsBrowserCompletion', 'sCmd=', sCmd);
		break;
		
		case js_const_addAttribute_symbol:
			oAJAXEngine.addNamedContext(js_const_addAttribute_symbol, '');
			oAJAXEngine.doAJAX('performAddGeonosisAttribute', 'displayAddOrDropAttributeCompletion', 'sCmd=', sCmd);
		break;
		
		case js_const_dropAttribute_symbol:
			oAJAXEngine.addNamedContext(js_const_addAttribute_symbol, '');
			oAJAXEngine.doAJAX('performDropGeonosisAttribute', 'displayAddOrDropAttributeCompletion', 'sCmd=', sCmd);
		break;
		
		case js_const_dropObject_symbol:
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performDropGeonosisObject', 'displayAddOrDropObjectCompletion', 'sCmd=', sCmd);
		break;
		
		case js_const_addObject_symbol:
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performAddGeonosisObject', 'displayAddOrDropObjectCompletion', 'sCmd=', sCmd);
		break;
		
		case js_const_dropClassDef_symbol:
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			var cObj = _$('selection_className');
			if (!!cObj) {
				cObj.disabled = true;
			}
			oAJAXEngine.doAJAX('performDropGeonosisClassDef', 'displayAddOrDropClassDefCompletion', 'sCmd=', sCmd);
		break;
		...
		case js_const_addClassDef_symbol:
			...
			oAJAXEngine.doAJAX('performAddGeonosisClassDef', 'displayAddOrDropClassDefCompletion', 'sCmd=', sCmd);
		break;
		
		case js_const_chgDataForObject_symbol:
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performSetGeonosisObjectData', 'displayObjectsBrowserEditCompletion', 'sCmd=', sCmd);
		break;
		
		case js_const_chgAttrsForObject_symbol:
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			oAJAXEngine.doAJAX('performSetGeonosisObjectAttribute', 'displayObjectsBrowserEditCompletion', 'sCmd=', sCmd);
		break;
		
		case js_const_getOBJECTS_symbol:
			pQueryString = '';
			oAJAXEngine.setContextName(js_const_Objects_symbol);
			var cObj = _$('div_objectsBrowser');
			if (!!cObj) {
			...
			oAJAXEngine.doAJAX('performGetGeonosisObjects', 'displayObjectsBrowser', sArgsQueryString);
		break;

		case js_const_Objects_symbol:
			pQueryString = '';
			oAJAXEngine.addNamedContext(js_const_Objects_symbol, pQueryString);
			oAJAXEngine.doAJAX('performGetGeonosisClasses', 'displayClassesBrowser', sArgsQueryString);
		break;
	}
}
...
function popCurrentlyEditedInput() {
	if (stack_current_span_inEdit.length > 0) {
		...
		var p_cObj = _$(pID);
		var p_dObj = _$(pID.asAltIDUsing('div'));
		if ( (!!p_cObj) && (!!p_dObj) ) {
...
function handleEditableGrid_onClick(cObj, qStringEnc) {
	...
	if (!!cObj) {
		var aSpec = qStringEnc.ezURLDecode();
		var dict = ezDictObj.get$(aSpec);
		var dObj = _$(cObj.id.asAltIDUsing('div'));
		var eObj = _$(cObj.id.asAltIDUsing('input'));
		if ( (!!dObj) && (!!eObj) ) {
			popCurrentlyEditedInput();

			var bool_isIdentity = isIdentity(cObj.id);

			eObj.value = cObj.innerHTML.ezStripHTML();
		...
		ezDictObj.remove$(dict.id);
	}
}

function _handleEditableGrid_onClick(id, qStringEnc) {
	return handleEditableGrid_onClick(_$(id), qStringEnc);
}

function processEditableGridInputAction(id, qStringEnc) {
	var cObj = _$(id.asAltIDUsing('span'));
	var dObj = _$(id.asAltIDUsing('div'));
	var eObj = _$(id); // the edit field that contains the updated data...
	if ( (!!cObj) && (!!dObj) && (!!eObj) ) {
		var aSpec = qStringEnc.ezURLDecode();
		var dict = ezDictObj.get$(aSpec);
		var ar = id.split('_');
...
function dictAsHTMLForBrowser(objType, dict) {
	...
	if ( (!!objType) && (!!dict) ) {
		...
		for (var i = 0; i < _keys.length; i++) {
			aKey = _keys[i];
			if (aKey.ezTrim().toUpperCase() != 'rowCnt'.toUpperCase()) {
				_html += '<tr>';
				...
				if (!!datum) {
					if (datum.length > 0) {
						...
						_html += '<span id="span_' + objType + '_' + aKey + '" class="textClass" style="display: inline; cursor:hand; cursor:pointer;" onmouseover="this.title = this.id;" onclick="handleEditableGrid_onClick(this,\'' + qString.ezURLEncode() + '\'); return false;">' + ((datum.length == 0) ? '_' : '&nbsp;' + datum) + '</span>';
						_html += '<div id="div_' + objType + '_' + aKey + '" style="display: none;">' + '<input id="input_' + objType + '_' + aKey + '" class="normalStatusBoldClass" size="30" maxlength="50" onkeydown="if (event.keyCode == 13) { processEditableGridInputAction(this.id,\'' + qString.ezURLEncode() + '\'); }; return true;">' + '</span>';
						_html += '</td>';
					}
				}
				_html += '</tr>';
...
function populateQuickBrowserPanel(qbObj, objType) {
	...
	if (!!qbObj) {
		...
		var dD = ezDictObj.get$(qbObj.options[sel].value)
		var qbDivObj = _$('div_' + objType + '_browser');
		if (!!qbDivObj) {
			_html = beginTable(objType, '&table=' + 'border="1"'.ezURLEncode() + '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
...
function populateQuickBrowser(objType, d) {
	...
	var qbObj = _$(_quickBrowseObjName);
	if ( (!!qbObj) && (!!d) ) {
		var _keys = d.getKeys();
		var datum = -1;
		var i = -1;
		...
		var aKey = '';
		var numCols = -1;
		var cObj = _$('selection_objectName');
		for (i = 0; i < _keys.length; i++) {
			...
			if (aKey.ezTrim().toUpperCase() != 'rowCnt'.toUpperCase()) {
				...
				if ( (objType.ezTrim().toUpperCase() == 'Attrs'.toUpperCase()) && (aKey.ezTrim().toUpperCase() == 'ATTRIBUTENAME'.toUpperCase()) ) {
					for (j = 0; j < datum.length; j++) {
						qString = d.asQueryStringForColumn(j);
						qbObj.options[qbObj.options.length] = new Option( datum[j], qString);
					}
					var btnObj = _$('btn_attr_remove');
					if ( (!!btnObj) && (!!cObj) ) {
					...
					if ( (!!datum) && (typeof datum == const_object_symbol) ) {
						if (datum.length > 0) {
							for (j = 0; j < datum.length; j++) {
								...
								_html += '<span id="span_' + objType + '_' + aKey + '_' + j + '" class="textClass" style="display: inline; cursor:hand; cursor:pointer;" onclick="handleEditableGrid_onClick(this,\'' + qString.ezURLEncode() + '\'); return false;">' + ((datum[j].length == 0) ? '_' : '&nbsp;' + datum[j]) + '</span>';
								_html += '<div id="div_' + objType + '_' + aKey + '_' + j + '" style="display: none;">' + '<input id="input_' + objType + '_' + aKey + '_' + j + '" class="normalStatusBoldClass" size="30" maxlength="50" onkeydown="if (event.keyCode == 13) { processEditableGridInputAction(this.id,\'' + qString.ezURLEncode() + '\'); }; return true;">' + '</span>';
								_html += '</td>';
							}
						}
					}
...
function performShowAddAttrPanel() {
	var cObj = _$('div_addAttr_panel');
	if (!!cObj) {
		cObj.style.display = const_inline_style;
	}
	var iObj = _$('input_addAttribute_attributeName');
	if (!!iObj) {
		ezSetFocus(iObj);
	}
}
...
function handle_onmousedown_beginDrag(draggableObj, el_dragtarget) {
	var dragBeginState = [];
	var dragTarget = _$(el_dragtarget);
 	var drObj = -1;
	if ( (!!dragTarget) && (!!draggableObj) && (!!DragObj) && (!!(drObj = DragObj.get$(draggableObj))) ) {
...
// +++
		...
		//	window.status = 'Event: ondragdrop !';
			drObj.removeDropTarget(dragTarget);
	 	};
	...
	if (!!cObj) {
		...
		if (!!dObj) {
			...
			if (sel > -1) {
				...
				_html += beginTable(_btnHtmlL + 'Attrs' + _btnHtmlR, '&table=' + 'border="1"'.ezURLEncode() + '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
				...
				oObj = _$('div_objectBrowser');
				if (!!oObj) {
					var _aDict = GeonosisObj.searchInstancesForID(cObj.options[sel].value);
// +++
					var dragHandle = '<div id="div_dragHandle_' + cObj.options[sel].value + '_' + ezUUID$() + '" title="This is a Draggable Object - Drag it to the Objects to Link Selection." onmousedown="handle_onmousedown_beginDrag(this, \'selection_objects2Link\'); return false;" onmouseover="handle_onmouseover_beginDrag(this); return false;" onmouseout="handle_onmouseout_beginDrag(this); return false;" style="border: thin dotted black;">' + 'Object: ' + cObj.options[sel].text + '</div>';
					_html = beginTable(dragHandle, '&table=' + 'border="1"'.ezURLEncode() + '&bool=' + 'includeDismissButton=true'.ezURLEncode() + '&div=' + 'dismissDiv=div_objectBrowser'.ezURLEncode() + '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&td=' + 'colspan="2"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
					_html += displayDictForBrowser('Objects', _aDict);
					_html += endTable();

					oObj.innerHTML = _html;
					
					ezDictObj.remove$(_aDict.id);
				}

				btnObj = _$('btn_objects_remove');
				if (!!btnObj) {
		...
		if (cObj.options[sel].flagged == null) {
			queueUp_AJAX_Sessions(js_const_getLinkedObjects_symbol, '&OBJECTID=' + cObj.options[sel].value.ezURLEncode());
			cObj.options[sel].flagged = true;
		}
	}
}
...
function handle_className_onChange(cObj) {
	dObj = _$('div_attrsBrowser');
	oObj = _$('div_objectBrowser');
	var oSelLinkedObjects = _$('selection_linkedObjects');
	if ( (!!cObj) && (!!dObj) && (!!oObj) ) { // cObj.options[sel].value is the OBJECTCLASSID
		...
		if (sel > -1) {
			...
			queueUp_AJAX_Sessions(js_const_getOBJECTS_symbol, '&CLASSNAME=' + cObj.options[sel].text.toString().ezURLEncode() + '&hasMetaData=' + (((objectsMetaDataStack.length > 0) && (objectAttributesMetaDataStack.length > 0)) ? 'true' : 'false').ezURLEncode());
		}
	}
}
...
function performShowAddObjectPanel() {
	var cObj = _$('div_addObject_panel');
	if (!!cObj) {
		cObj.style.display = const_inline_style;
	}
	var iObj = _$('input_addObject_objectName');
	if (!!iObj) {
		ezSetFocus(iObj);
	}
}
...
function performConfirmDropAttributeDialog() {
	var sel = -1;
	var cObj = _$('selection_attrs_browser');
	...
	if (!!cObj) {
		if ((sel = cObj.selectedIndex) > -1) {
			...
			if (xBool) {
				var _CLASSNAME = '';
				var cnObj = _$('selection_className');
				if ( (!!cnObj) && ((sel = cnObj.selectedIndex) > -1) ) {
					_CLASSNAME = cnObj.options[sel].text;
				}
				var qStr = attrID + '&CLASSNAME=' + _CLASSNAME + '&hasMetaData=' + (((objectsMetaDataStack.length > 0) && (objectAttributesMetaDataStack.length > 0)) ? 'true' : 'false').ezURLEncode();
	...
	if (!!cObj) {
		if ((sel = cObj.selectedIndex) > -1) {
			...
			if (xBool) {
				...
				queueUp_AJAX_Sessions(js_const_dropObject_symbol, '&objectID=' + objectID.ezURLEncode() + '&CLASSNAME=' + _CLASSNAME + '&hasMetaData=' + (((objectsMetaDataStack.length > 0) && (objectAttributesMetaDataStack.length > 0)) ? 'true' : 'false').ezURLEncode());
			}
		}
	}
}
...
function processAddObjectInputAction(id, qStr) {
	...
	for (i = 0; i < stack_addObject_inEdit.length; i++) {
		iObj = _$(stack_addObject_inEdit[i]);
		if ( (!!iObj) && (iObj.value.ezTrim().length > 0) ) {
			kValuesCnt++;
		}
		if (currentNum == -1) {
	...
	if (kValuesCnt == stack_addObject_inEdit.length) {
		...
		queueUp_AJAX_Sessions(js_const_addObject_symbol, '&objectClassID=' + objectClassID + '&CLASSNAME=' + _CLASSNAME + '&hasMetaData=' + (((objectsMetaDataStack.length > 0) && (objectAttributesMetaDataStack.length > 0)) ? 'true' : 'false').ezURLEncode() + qString);
		...
		iObj = _$(stack_addObject_inEdit[currentNum]);
		if (!!iObj) {
			ezSetFocus(iObj);
		}
	}
...
function performSaveAttribute() {
	...
	for (i = 0; i < stack_addAttribute_inEdit.length; i++) {
		iObj = _$(stack_addAttribute_inEdit[i]);
		if (!!iObj) {
			iObj.style.border = 'thin solid green';
			var ar = iObj.id.split('_');
			qString += '&' + ar[ar.length - 1] + '=' + escape(iObj.value);
	...
	var _qStr = '&objectID=' + objectID + '&OBJECTNAME=' + _OBJECTNAME + '&hasMetaData=' + (((objectsMetaDataStack.length > 0) && (objectAttributesMetaDataStack.length > 0)) ? 'true' : 'false').ezURLEncode() + qString;
	queueUp_AJAX_Sessions(js_const_addAttribute_symbol, _qStr);
}

function processAddAttributeInputAction(id, qStr) {
	var btnObj = _$('btn_attr_submit');
	var oAttributeName = _$('input_addAttribute_attributeName');
	var oValueString = _$('input_addAttribute_valueString');
	var oValueText = _$('input_addAttribute_valueText');
	var oDisplayOrder = _$('input_addAttribute_displayOrder');
	var oStartVersion = _$('input_addAttribute_startVersion');
	var oLastVersion = _$('input_addAttribute_lastVersion');
	var oCreated = _$('input_addAttribute_created');
	var oCreatedBy = _$('input_addAttribute_createdBy');
	var oUpdated = _$('input_addAttribute_updated');
	var oUpdatedBy = _$('input_addAttribute_updatedBy');
	if ( (!!btnObj) && (!!oAttributeName) && (!!oValueString) && (!!oValueText) && (!!oDisplayOrder) && (!!oDisplayOrder) && (!!oLastVersion) ) {
		btnObj.disabled = (( (oAttributeName.value.ezTrim().length > 0) && ( (oValueString.value.ezTrim().length > 0) || (oValueText.value.ezTrim().length > 0) ) && (oDisplayOrder.value.ezTrim().length > 0) && (oStartVersion.value.ezTrim().length > 0) && (oLastVersion.value.ezTrim().length > 0) && (oCreated.value.ezTrim().length > 0) && (oCreatedBy.value.ezTrim().length > 0) && (oUpdated.value.ezTrim().length > 0) && (oUpdatedBy.value.ezTrim().length > 0) ) ? false : true);
	}
}
...
function displayObjectsBrowserGUI() {
	...
	if ( (!!dObj) && (!!cObj) ) {
		...
		if (sel > -1) {
			...
			_html += beginTable(_btnHtmlL + 'Objects' + _btnHtmlR, '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
			_html += '<tr>' + '<td>';
			...
			_html += beginTable('Add an Object', '&bool=' + 'includeDismissButton=true'.ezURLEncode() + '&div=' + 'dismissDiv=div_addObject_panel'.ezURLEncode() + '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
			_html += '<tr>' + '<td>';
			_html += beginTable();
			...
			_html += beginTable(_btnHtmlL + 'Add an Attribute', '&bool=' + 'includeDismissButton=true'.ezURLEncode() + '&div=' + 'dismissDiv=div_addAttr_panel'.ezURLEncode() + '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
			_html += '<tr>' + '<td>';
			_html += beginTable();
...
function handleGetAttributesForOBJECT(qObj) {
	...
	if (currentlySelectedObject_sel > -1) {
		var oSel = _$('selection_objectName');
		if (!!oSel) {
...
function processAddClassInputAction(id) {
	...
	for (i = 0; i < stack_addClass_inEdit.length; i++) {
		iObj = _$(stack_addClass_inEdit[i]);
		if ( (!!iObj) && (iObj.value.ezTrim().length > 0) ) {
			kValuesCnt++;
		}
		if (currentNum == -1) {
	...
	if (kValuesCnt == stack_addClass_inEdit.length) {
		...
		iObj = _$(stack_addClass_inEdit[currentNum]);
		if (!!iObj) {
			ezSetFocus(iObj);
		}
	}
...
function performShowAddClassPanel() {
	var cObj = _$('div_addClass_panel');
	if (!!cObj) {
		cObj.style.display = const_inline_style;
	}
	var iObj = _$('input_addClass_className');
	if (!!iObj) {
		ezSetFocus(iObj);
	}
}
...
function performConfirmDropClassDialog() {
	...
	if (!!cObj) {
		if ((sel = cObj.selectedIndex) > -1) {
			...
			if (xBool) {
				queueUp_AJAX_Sessions(js_const_dropClassDef_symbol, '&objectClassID=' + classID.ezURLEncode());
			}
		}
	}
}
...
function handle_onChange_objects2Link(oSel) {
	...
	var sParms = '';
	var selectedOptions = [];
	var oBtnAdd = _$('btn_objectLink_add');
	var oBtnDrop = _$('btn_objectLink_drop');
	if ( (!!oSel) && (!!oBtnAdd) && (!!oBtnDrop) ) {
		for (i = 0; i < oSel.options.length; i++) {
			if (oSel.options[i].selected) {
				selectedOptions.push(oSel.options[i]);
			}
		}
		oBtnDrop.disabled = ((selectedOptions.length == 0) ? true : false);
		...
		//	oBtnAdd.disabled = ((selectedOptions.length == 2) ? false : true);
		}
		if (selectedOptions.length == 2) {
			for (i = 0; i < selectedOptions.length; i++) {
				sParms += '&OBJECTID' + (i + 1) + '=' + selectedOptions[i].value.ezURLEncode();
...
function performDropLinkObjects() {
	var oSel = _$('selection_objects2Link');
	if (!!oSel) {
...
function displayClassesBrowser(qObj) {
	...
	function getDictFromQuery(_ri, _dict) {
		if (qDict == -1) {
			qDict = ezDictObj.get$(_dict.asQueryString());
		}
	}
	...
	function displayClassBrowser(qO, nR, bool_justRefresh) {
		...
		if (!!qData) {
			if (bool_justRefresh) {
				...
				html += beginTable(_btnHtmlL + '&nbsp;' + 'Classes' + '&nbsp;' + _btnHtmlR, '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
				html += '<tr>' + '<td>';
				...
				html += beginTable('Add a Class', '&bool=' + 'includeDismissButton=true'.ezURLEncode() + '&div=' + 'dismissDiv=div_addClass_panel'.ezURLEncode() + '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
				html += '<tr>' + '<td>';
				html += beginTable();
...
//	ezAlert('qObj.names = [' + qObj.names + ']' + '\n========================================\n');
//	for (i = 0; i < qObj.names.length; i++) {
//		ezAlert('qObj.data[' + qObj.names[i] + '] = [' + qObj.data[qObj.names[i]] + ']' + '\n========================================\n');
//	}
	var bc = -1;
	if (nRecs > 0) {
		var bcAR = [];
		...
		if (!!oParms) {
			try {
				bc = oParms.getValueFor('breadCrumbs');
			} catch (e) { oParms.iterateRecObjs(getDictFromQuery); bc = qDict.getValueFor('breadCrumbs'); };
			if (!!bc) {
	...
	//			ezAlert('+++\n' + 'cObj = [' + cObj + ']' + ', cObj.style.width = [' + cObj.style.width + ']' + ', cObj.style.height = [' + cObj.style.height + ']' + ', cObj.innerHTML.length = [' + cObj.innerHTML.length + ']');
				...
				if (cObj.innerHTML.length == 0) {
					...
					var _btnHtmlL = '<button id="btn_objectLink_drop" disabled class="buttonClass" title="Click this button to remove the selected object(s) from the Objects to Link list." onclick="performDropLinkObjects(); return false;">[-]</button>&nbsp;';
					_html += beginTable(_btnHtmlL + 'Objects to Link' + _btnHtmlR, '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
					_html += '<tr>' + '<td>';
				...
				//	var _btnHtmlL = '<button id="btn_attr_remove" disabled class="buttonClass" title="Click this button to remove the selected Attribute." onclick="performConfirmDropAttributeDialog(); return false;">[-]</button>&nbsp;';
					_html += beginTable('Linked Objects', '&tr=' + 'bgcolor="silver"'.ezURLEncode() + '&td=' + 'align="center"'.ezURLEncode() + '&span=' + 'class="boldPromptTextClass"'.ezURLEncode());
					_html += '<tr>' + '<td>';
...
function displayObjectsBrowserEditCompletion(qObj) {
	...
	if (nRecs > 0) {
		...
		if (!bool_isAnyErrorRecords) {
			...
			var cObj = _$('selection_className');
			var oObj = _$('selection_objectName');
			if ( (!!cObj) && (!!oObj) ) {
				...
				if ( (cSel > -1) && (oSel > -1) ) {
					...
					if ( (eAR.length == 3) || (eAR.length == 4) ) {
						...
						if (eAR.length == 4) {
							...
							if (sel > -1) {
								var datumDict = ezDictObj.get$(qbObj.options[sel].value);
						...
						var eObj = _$(bcAR.asAltIDUsing('input'));
						var gObj = _$(bcAR);
...
function displayLinkedObjectsBrowserCompletion(qObj) {
	...
	var j = -1;
	var _contextName = js_const_Objects_symbol;
	var linksDict = ezDictObj.get$();
	var oSelLinkedObjects = _$('selection_linkedObjects');
	var oSelObjects2Link = _$('selection_Objects2Link');
	var argsDict = ezDictObj.get$();
		...
		if (_id == null) {
			...
			if ( (!!oSelLinkedObjects) && (!!oSelObjects2Link) ) {
				...
				for (i = 0; i < oSelObjects2Link.options.length; i++) {
					if (oSelObjects2Link.options[i].flagged == null) {
						queueUp_AJAX_Sessions(js_const_getLinkedObjects_symbol, '&OBJECTID=' + oSelObjects2Link.options[i].value.ezURLEncode() + '&DONOTCLEARLINKEDOBJECTS=1');
						oSelObjects2Link.options[i].flagged = true;
					}
				}
...
//_alert('============================================================\n');
	...
	if (nRecs > 0) {
		...
		if (!bool_isAnyErrorRecords) {
			...
			btnObj = _$('btn_attrs_add');
			if (!!btnObj) {
				btnObj.disabled = true;
			}
			
			btnObj = _$('btn_attr_remove');
			if (!!btnObj) {
				btnObj.disabled = true;
			}
			
			var qbObj = _$('selection_Attrs_browser');
			if (sCmd.toUpperCase() == js_const_dropAttribute_symbol.toUpperCase()) {
				if (!!qbObj) {
					...
					for (i = 0; i < qbObj.options.length; i++) {
						qbDict = ezDictObj.get$(qbObj.options[i].value);
