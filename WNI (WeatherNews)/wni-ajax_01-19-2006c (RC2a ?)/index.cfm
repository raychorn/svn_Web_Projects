...
// +++
			function dropItemFrom(item, anObj) {
				...
				if ( (!!anObj) && (!!anObj.options) && (!!anObj.selectedIndex) ) {
					alert('item = [' + item + ']' + ', anObj.id = [' + anObj.id + ']');
					for (i = 0; i < anObj.options.length; i++) {
						if (parseInt(anObj.options[i].value) == parseInt(item)) {
			...
			function onChange_dispOrder_EventHandler(oObj) {
				...
				if ( (!!oObj) && (!!oObj.options) && (!!oObj.selectedIndex) ) {
					...
					m = parseInt(ar[ar.length - 1]);
					for (i = 1; i <= m; i++) {
						ar[ar.length - 2] = i;
						newID = ar.join('_');
						cObj = getGUIObjectInstanceById(newID);
						if (!!cObj) {
							if (n != i) {
								_db += ' [-] ';
								dropItemFrom(i, cObj);
							}
							_db += ((n == i) ? '(*) ' : '(-)') + 'newID = [' + newID + ']' + ', cObj = [' + cObj + ']' + '\n';
						}
					}
					_alert('onChange_dispOrder_EventHandler() :: ' + 'oObj.id = [' + oObj.id + ']' + ', aValue = [' + aValue + ']' + ', ar = [' + ar + ']' + '\n' + _db);
				}
			 }
			...
			function displayMenuGroupNames(qObj) {
				...
				function displayRecord(_ri, _dict, _rowCntName, queryObj) {
					...
					function htmlDisplayOrderControls(ri, n, m) {
						var i = -1;
						var n_ch = -1;
						var m_ch = -1;
						var html = '';
						...
						html += '<select id="select_menuGroupRecord_disporder_' + ri + '_' + m + '" class="textClass" onchange="onChange_dispOrder_EventHandler(this); return false;">';
					...
					try {
						...
						_DISPORDER = htmlDisplayOrderControls(_ri, _dict.getValueFor('DISP_ORDER'), (((!!queryObj) && (typeof queryObj == const_object_symbol)) ? queryObj.recordCount() : -1));
