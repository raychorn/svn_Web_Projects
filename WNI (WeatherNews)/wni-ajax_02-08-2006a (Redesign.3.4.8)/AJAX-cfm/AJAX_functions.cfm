...
<cfsavecontent variable="sql_qGetLayerEditorContents">
	<cfoutput>
		SELECT id, layerName, layerDisplayName, timeFileList, timeFileType
		FROM dbo.AvnLayer 
		ORDER BY layerName
 	</cfoutput>
</cfsavecontent>
	...
	switch (Request.qryStruct.cfm) {
		case 'getLayerEditorContents':
			...
			if (NOT Request.dbError) {
				Request.cf_dump(Request.qGetLayerEditorContents, 'Request.qGetLayerEditorContents', false);
...
//Request.cf_dump(_ar, '1. _ar newDispOrder = [#newDispOrder#], self_id = [#self_id#]', false);
								try {
									ArrayInsertAt( _ar, newDispOrder + 1, self_id);
								} catch (Any e) {
									_ar[ArrayLen(_ar) + 1] = self_id;
								}
//Request.cf_dump(_ar, '2. _ar newDispOrder = [#newDispOrder#]', false);

								for (i = 1; i lte ArrayLen(_ar); i = i + 1) {
//writeOutput('<b>_ar[#i#] = [#_ar[i]#]</b><br>');
