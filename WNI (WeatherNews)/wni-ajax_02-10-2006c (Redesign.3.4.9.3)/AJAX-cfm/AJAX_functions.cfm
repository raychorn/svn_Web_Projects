	...
	<cfoutput>
		SELECT e1.id, CASE e1.srcTableName WHEN 'AvnMenu' THEN
			...
			(SELECT DISTINCT menuUUID
			  ...
			  WHERE (id = e1.parent_id)) AS parentUUID, e1.dispOrder, AvnMenuEditorOptions.rb_options, AvnMenuEditorOptions.link_options
...
<cfscript>
	...
	switch (Request.qryStruct.cfm) {
		...
		//	QueryAddRow(qObj, 1);
		//	QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
		//	QuerySetCell(qObj, 'msg', 'Request.qryStruct.argCnt = [#Request.qryStruct.argCnt#]', qObj.recordCount);
