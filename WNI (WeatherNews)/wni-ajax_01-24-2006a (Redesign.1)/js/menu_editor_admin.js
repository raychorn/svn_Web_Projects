...
function _processMenuCutToClipboard(brObj, bool_copy_to_clipboard) {
	...
	if ( (!!brObj) && (!!clipObj) && (!!bcutObj) && (!!bcopyObj) && (!!bcpasteObj) && (!!pgObj) ) {
		...
		var c = retrieveMenuAtLevel(global_menuEditorObj.menuArray, _menu_stack, 0);
		...
		for (var i = (_menu_stack.length == 0) ? 0 : 1; i < mm.length; i++) {
			if (mm[i] != null) {
				if (typeof mm[i] == const_object_symbol) {
					var c = mm[i];
					_prompt = '(' + retrieveMenuPrompt(c).URLDecode().clipCaselessReplace(global_menuEditorObj.old_Ellipsis_symbol, '') + ')';
				} else {
					toks = mm[i].split('|');
					_prompt = '';
					if (toks.length > 2) {
...
function initMenuEditorClipboardObjects() {
	...
	//	html += '</p>';
		...
		if (sObj != null) {
			...
			//	alert('ERROR: Programming Error - Missing div named (clipboard_paste_before).');
			}
			if (bfontObj != null) {
				bfontObj.color = 'silver';
			} else {
			//	alert('ERROR: Programming Error - Missing div named (clipboard_paste_before_font).');
			}
			if (afterObj != null) {
				afterObj.checked = false;
				afterObj.disabled = true;
			} else {
			//	alert('ERROR: Programming Error - Missing div named (clipboard_paste_after).');
			}
			if (afontObj != null) {
				afontObj.color = 'silver';
			} else {
			//	alert('ERROR: Programming Error - Missing div named (clipboard_paste_after_font).');
			}
		} else {
