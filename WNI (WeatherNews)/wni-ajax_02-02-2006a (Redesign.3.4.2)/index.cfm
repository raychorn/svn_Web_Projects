...
// +++			
			...
			function displayMenuEditor(qObj) {
				...
				function displayRecord(_ri, _dict, _rowCntName, queryObj) {
					...
					var bool_isAddingMenuItem = (_SRCTABLENAME == 'AvnLayer');
					var bool_debugMode = bool_isAddingMenuItem;
					...
					try {
						...
						if (_PARENT_ID > -1) {
							...
							global_menuEditorObj.debugMode = bool_debugMode;
							global_menuEditorObj.navigateToUUID(_PARENTUUID, true);
							global_menuEditorObj.deleteDefaultPlaceHolder();
						//	_alert('A. _PARENT_ID = [' + _PARENT_ID + ']' + ', _PARENTUUID = [' + _PARENTUUID + ']' + ', _PROMPT = [' + _PROMPT + ']' + ', _DISPORDER = [' + _DISPORDER + ']' + ', bool_isAddingMenuItem = [' + bool_isAddingMenuItem + ']');
						...
						//	_alert('B. _PARENT_ID = [' + _PARENT_ID + ']' + ', _PARENTUUID = [' + _PARENTUUID + ']' + ', _PROMPT = [' + _PROMPT + ']' + ', _DISPORDER = [' + _DISPORDER + ']' + ', _do = [' + _do + ']' + ', brObj.selectedIndex = [' + brObj.selectedIndex + ']' + ', _SRCTABLENAME = [' + _SRCTABLENAME + ']');
							if (bool_isAddingMenuItem) {
								menuEditorObj.processMenuItemAdditor(brObj, global_menuEditorObj);
							} else {
							...
							alert('ERROR - Programming Error : Missing (menu_browser).');
						}
					} catch(e) {
