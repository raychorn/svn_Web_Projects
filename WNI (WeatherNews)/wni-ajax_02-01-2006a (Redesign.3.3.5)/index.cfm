...
<cfset _ajax_engine_js = "#_js_path#ajax_engine.js">
<cfset _ajax_obj_js = "#_js_path#ajax_obj.js">
<cfset _jsapi_extensions_js = "#_js_path#jsapi_extensions.js">
...
<cfoutput>
	...
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<script language="JavaScript1.2" src="#_ajax_engine_js#" type="text/javascript">></script>
		...
		<!--
			...
			//	jsCode += "execJSAPI_func('getLayerGroupNames', 'displayLayerGroupNames(" + oAJAXEngine.js_global_varName + ")'); ";
			...
			//	jsCode += "execJSAPI_func('getMenuGroupNames', 'displayMenuGroupNames(" + oAJAXEngine.js_global_varName + ")'); ";
			//	register_jsapi_function(jsCode);
				...
				jsCode += "execJSAPI_func('getMenuEditorContents', 'displayMenuEditor(" + oAJAXEngine.js_global_varName + ")'); ";
				register_jsapi_function(jsCode);
				...
				jsCode += "execJSAPI_func('getGroupNames', 'displayTopLevelMenuNames(" + oAJAXEngine.js_global_varName + ")'); ";
				register_jsapi_function(jsCode);
				...
				jsCode += "execJSAPI_func('getLayerNames', 'displayTopLevelMenuNames(" + oAJAXEngine.js_global_varName + ")'); ";
				register_jsapi_function(jsCode);
// +++
				...
				execJSAPI_func('getTopLevelMenuNames', 'displayTopLevelMenuNames(' + oAJAXEngine.js_global_varName + ')');
			}
			...
			function perform_click_btn_addMenuName(_spanName, _cfm_add_cmd, _cfm_save_cmd) {
				...
				if (dataVals.length > 0) {
					if (bool_performing_btn_addMenuName) {
						execJSAPI_func(_cfm_add_cmd + '_v', 'displayTopLevelMenuNames(' + oAJAXEngine.js_global_varName + ')', dataVals.length, dataVals);
					} else {
						execJSAPI_func(_cfm_save_cmd + '_v', 'displayTopLevelMenuNames(' + oAJAXEngine.js_global_varName + ')', dataVals.length, dataVals);
					}
				} else {
					...
					if (!!eObj) {
						if (eObj.value.trim().length > 0) {
							if (bool_performing_btn_addMenuName) {
								execJSAPI_func(_cfm_add_cmd, 'displayTopLevelMenuNames(' + oAJAXEngine.js_global_varName + ')', eObj.value.trim());
							} else {
								execJSAPI_func(_cfm_save_cmd, 'displayTopLevelMenuNames(' + oAJAXEngine.js_global_varName + ')', eObj.value.trim(), eObj.title.trim());
							}
						} else {
			...
			function perform_click_btn_dropMenuName(_objID_prefix, _iNum, _recID, _spanName, _cfm_drop_cmd) {
				...
				if (confirm('Are you sure you really want to delete that record from the database ?')) {
					...
					if (_recID > -1) {
						oAJAXEngine.setContextName(_spanName);
						execJSAPI_func(_cfm_drop_cmd, 'global_guiActionsObj.popAll(); displayTopLevelMenuNames(' + oAJAXEngine.js_global_varName + ')', _recID);
					} else {
			...
			function clickEventHandler_Record(sData, aContextName, aRecID) {
				var mObj = getGUIObjectInstanceById('_menu_item_editor_prompt');
				if (!!mObj) {
					mObj.value = sData;
			...
			function performAssociateMenu2Group() {
				...
				if ( (!!cObj1) && (!!cObj2) ) {
					...
					execJSAPI_func('saveMenuGroupAssociation', 'displayMenuGroupNames(' + oAJAXEngine.js_global_varName + ')', global_dict.getValueFor(menuName), global_dict.getValueFor(groupName));
				}
			}
			...
			function perform_click_btn_dropMenuGroupName(_objID_prefix, _iNum, _recID, _spanName) {
				...
				if (confirm('Are you sure you really want to delete that record from the database ?')) {
					...
					if (_recID > -1) {
						oAJAXEngine.setContextName(_spanName);
						execJSAPI_func('dropMenuGroupAssociation', 'global_guiActionsObj.popAll(); displayMenuGroupNames(' + oAJAXEngine.js_global_varName + ')', _recID);
					} else {
						alert('ERROR: Programming ERROR - This message should NEVER ever been seen so this means someone, shall we call him/her the programmer ?!?  Forgot something.  Oops !');
					}
				} else {
			...
			function onChange_dispOrder_EventHandler(oObj) {
				...
				if ( (!!oObj) && (!!oObj.options) && (!!oObj.selectedIndex) ) {
					...
					oAJAXEngine.setContextName('#const_menuGroupRecord_symbol#');
					execJSAPI_func('setMenuGroupDispOrder', 'displayMenuGroupNames(' + oAJAXEngine.js_global_varName + ')', oObj.id, aValue);
				}
			 }
...
// +++			
			function _onDeleteMenuSomething(v) {
				var x = v.toString();
				var iAR = x.split(',');
				var i = -1;
				var oAR = [];
				...
				for (i = 0; i < iAR.length; i++) {
					ar = iAR[i].toString().split('|');
					if (ar.length > 1) {
						t = ar[1].toString().trim();
						if ( (ar.length == 3) && (t.length > 0) ) {
							oAR.push(t);
						}
					}
				}
			...
			function displayMenuEditor(qObj) {
				...
				function displayRecord(_ri, _dict, _rowCntName, queryObj) {
					...
					var v = '';
					try {
						global_menuEditorObj._uuid = function () { return _MENUUUID; };
						if (_PARENT_ID > -1) {
							// BEGIN: Navigate to the parent this node needs to be put into... then navigate back...
							global_menuEditorObj.navigateToUUID(_PARENTUUID, true);
							global_menuEditorObj.deleteDefaultPlaceHolder();
						//	if (_PARENTUUID == '11387398467340') _alert('A. _PARENT_ID = [' + _PARENT_ID + ']' + ', _PARENTUUID = [' + _PARENTUUID + ']' + ', _PROMPT = [' + _PROMPT + ']' + ', _DISPORDER = [' + _DISPORDER + ']' + '\n' + global_menuEditorObj.menuArray);
							// END! Navigate to the parent this node needs to be put into... then navigate back...
						}
						var brObj = getGUIObjectInstanceById('menu_browser'); 
						if (!!brObj) { 
							var _do = _DISPORDER - 1;
							if (_do < 0) {
								_do = 0;
							}
							if (brObj.selectedIndex != _do) {
								brObj.selectedIndex = _do;
							}
						//	if (_PARENTUUID == '11387398467340') _alert('B. _PARENT_ID = [' + _PARENT_ID + ']' + ', _PARENTUUID = [' + _PARENTUUID + ']' + ', _PROMPT = [' + _PROMPT + ']' + ', _DISPORDER = [' + _DISPORDER + ']' + ', brObj.selectedIndex = [' + brObj.selectedIndex + ']');
							menuEditorObj.processMenuSubMenuAdditor(brObj, global_menuEditorObj);
							var prObj = getGUIObjectInstanceById('_menu_item_editor_prompt'); 
							if (!!prObj) { 
								prObj.value = _PROMPT.stripIllegalChars();
								menuEditorObj.processSaveMenuItemEditor(global_menuEditorObj);
							}
							global_menuEditorObj.navigateBackFromUUID(brObj);
						}
					} catch(e) {
						jsErrorExplainer(e, 'A. Debug', true);
					} finally {
					}
				};
						...
						//	_alert(oParms);
							...
							if (!!cObj) {
								...
								global_menuEditorObj.onCommitSubMenu = function (v, offset) {
									...
									if (!!srcId) {
									//	alert(aDict);
										execJSAPI_func('onCommitSubMenu', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), srcId, aDict.getValueFor('menuUUID'), aDict.getValueFor('parentId'), aDict.getValueFor('dispOrder'));
									} else {
								...
								global_menuEditorObj.onCommitMenuItem = function (v) {
									var aDict = _onCommitMenuSomething(v, currentParentSubMenu_id);
									var srcId = aDict.getValueFor('srcId');
									if (!!srcId) {
										execJSAPI_func('onCommitMenuItem', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), srcId, aDict.getValueFor('menuUUID'), aDict.getValueFor('parentId'), aDict.getValueFor('dispOrder'));
									} else {
								...
								global_menuEditorObj.onPasteSubMenu = function (v, offset) {
									var aDict = _onCommitMenuSomething(v, currentParentSubMenu_id, offset);
									var srcId = aDict.getValueFor('srcId');
									if (!!srcId) {
									//	alert(aDict);
										execJSAPI_func('onPasteSubMenu', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('menuUUID'), aDict.getValueFor('dispOrder'));
									} else {
										alert('ERROR - Programming Error - srcId (' + srcId + ') is undefined or missing in onPasteSubMenu() callback function.');
									}
									_alertM('global_menuEditorObj.onPasteSubMenu(v = [' + v + '], offset = [' + offset + '])');
									DictionaryObj.removeInstance(aDict.id);
								}
								
								global_menuEditorObj.onPasteMenuItem = function (v, offset) {
									_alertM('global_menuEditorObj.onPasteMenuItem(v = [' + v + '], offset = [' + offset + '])');
								}
								...
								global_menuEditorObj.onChangeSubMenu = function (v, offset) {
									...
									if (!!srcId) {
									//	alert(aDict);
										execJSAPI_func('onChangeSubMenu', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), srcId, aDict.getValueFor('menuUUID'), aDict.getValueFor('parentId'), aDict.getValueFor('dispOrder'));
									} else {
										alert('ERROR - Programming Error - srcId (' + srcId + ') is undefined or missing in onCommitSubMenu() callback function.');
									}
								...
								global_menuEditorObj.onOpenSubMenu = function (v) {
									var aDict = _onCommitMenuSomething(v, currentParentSubMenu_id);
									currentParentSubMenu_id = aDict.getValueFor('menuUUID');
									_alertM('global_menuEditorObj.onOpenSubMenu(v = [' + v + '])' + ', currentParentSubMenu_id = [' + currentParentSubMenu_id + ']');
									DictionaryObj.removeInstance(aDict.id);
								}
								
								global_menuEditorObj.onCloseSubMenu = function (v) {
									currentParentSubMenu_id = -1;
									_alertM('global_menuEditorObj.onCloseSubMenu(v = [' + v + '])' + ', currentParentSubMenu_id = [' + currentParentSubMenu_id + ']');
								}
								...
								global_menuEditorObj.onDeleteMenuItem = function (v) {
									var ar = _onDeleteMenuSomething(v);
									if ( (!!ar) && (ar.length > 0) ) {
									//	alert(ar);
										execJSAPI_func('onDeleteMenuItem', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', ar);
									}
							...
							//	global_menuEditorObj.bool_debugMode = true;
							} else {
								alert('ERROR: Programming error, the element known as (' + 'div_contents_' + oAJAXEngine.currentContextName + ') is not present in the document.');
							}
						}
			...
			<tr>
				...
				<td>
					<table width="100%" cellpadding="-1" cellspacing="-1">
						...
						<tr>
							<td id="td_menuHelperPanel" align="center" style="display: none;">
								<table width="100%" cellspacing="-1" cellpadding="-1" id="table_menuHelperPanel" style="width: 800px;">
									<tr>
										<td align="left" valign="top">
											<textarea id="ta_menuHelperPanel" readonly rows="5" cols="180" class="textClass"></textarea>
										</td>
									</tr>
								</table>
							</td>
