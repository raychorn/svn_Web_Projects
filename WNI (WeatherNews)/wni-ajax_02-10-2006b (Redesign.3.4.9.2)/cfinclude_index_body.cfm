<cfoutput>
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<!--
			...
			oMenuDict.bool_returnArray = true;
			var uMenuDict = DictionaryObj.getInstance();
			uMenuDict.bool_returnArray = true;
...
//_alert('\n\naDict = [' + aDict + ']');
						...
						if (id == 'cb_menu_options_useRB') {
							if (sel > -1) {
								...
								if (typeof ar == const_object_symbol) {
									...
									//	_alert('handleMenuOptionsClicks(brObj = [' + brObj + '], id = [' + id + '])' + ', cbObj.value = [' + cbObj.value + ']' + ', sel = [' + sel + ']' + '\n\ntxt = [' + txt + ']' + ', val = [' + val + ']' + '\n\n_menuEditorObj = [' + _menuEditorObj + ']' + ', ar = [' + ar + ']' + ', oMenuDict.bool_returnArray = [' + oMenuDict.bool_returnArray + ']' + '\n\noAJAXEngine.currentContextName = [' + oAJAXEngine.currentContextName + ']');
									} else {
									...
									alert('ERROR.3: Programming Error - Invalid data type (' + (typeof ar) + '), expected "object" in handleMenuOptionsClicks().');
								}
							} else {
							...
							if (!bool_isAnyErrorRecords) {
								...
								if (!!cObj) {
									...
									global_menuEditorObj.onClickMenuItem = function (brObj, bdelObj) {
										...
										var cbObj = getGUIObjectInstanceById('cb_menu_options_useRB');
										if (!!cbObj) {
											cbObj.checked = false;
										}
						...
						//	showAbstractPanel(true);  // +++
							...
							if (!!dObj) {
								...
								try {
									...
									_alert('global_ClientMenuObj.oMenuDict = [' + global_ClientMenuObj.oMenuDict + ']');
									_alert('qData = [' + qData + ']');
			...
			function getEditableFieldSizeFor(_colName) {
				return ((_colName.toUpperCase() == 'LAYERDISPLAYNAME') ? 50 : ((_colName.toUpperCase() == 'LAYERNAME') ? 50 : ((_colName.toUpperCase() == 'TIMEFILELIST') ? 150 : ((_colName.toUpperCase() == 'TIMEFILETYPE') ? 10 : -1))));
			}
			
			function getEditableFieldMaxLengthFor(_colName) {
				return ((_colName.toUpperCase() == 'LAYERDISPLAYNAME') ? 50 : ((_colName.toUpperCase() == 'LAYERNAME') ? 50 : ((_colName.toUpperCase() == 'TIMEFILELIST') ? 150 : ((_colName.toUpperCase() == 'TIMEFILETYPE') ? 15 : -1))));
			}
			...
			function processLayerEditorGridInputAction(id) {
				...
				if (!!cObj) {
					...
					doAJAX_func('updateLayerData', 'updatedLayerData(' + oAJAXEngine.js_global_varName + ')', cObj.value.URLEncode(), _ar);
					cObj.disabled = true;
				}
			}
							...
							if (typeof arguments[i] == const_object_symbol) {
								...
								try {
									...
									if (rowCnt == 1) {
										...
										html += '<td>';
										datum = '@@';
										html += '<span class="textClass">' + datum + '</span>';
										html += '</td>';
										html += endTable();
										html += '</td>';
										html += '</tr>';
													...
													<td>
														<table id="table_abstract_dismiss_button" width="100%" style="display: none;">
															<tr>
																...
																<td align="right">
																	<button class="buttonMenuClass" onclick="this.disabled = true; showAbstractPanel(false); var cObj = getGUIObjectInstanceById('div_contents'); if (!!cObj) { flushGUIObjectChildrenForObj(cObj); cObj.innerHTML = ''; }; return false;">[X]</button>
																</td>
															</tr>
														</table>
													</td>
							...
							<td>
								<table id="table_abstract_dismiss_button2" width="100%" style="display: none;">
									<tr>
										...
										<td align="right">
											<button class="buttonMenuClass" onclick="this.disabled = true; showAbstractPanel2(false); var cObj = getGUIObjectInstanceById('div_layerEditorTest'); if (!!cObj) { flushGUIObjectChildrenForObj(cObj); cObj.innerHTML = ''; }; return false;">[X]</button>
										</td>
									</tr>
								</table>
							</td>
