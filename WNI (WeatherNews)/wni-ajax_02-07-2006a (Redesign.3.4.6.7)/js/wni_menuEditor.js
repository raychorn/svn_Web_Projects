	...
	openClientSubMenuByUUID : function(aUUID, uuids) {
		...
		if (!!dObj) {
			if (dObj.innerHTML.length > 0) {
				...
				_html = '<table width="' + (156 + 20) + '" cellpadding="-1" cellspacing="-1" style="margin-left: -20px;">';
				for (i = 0; i < iAR.length; i++) {
					...
					if (hasChildren) {
						...
						_html += '<td align="left">';
						_html += '<span class="menuClass" style="margin-left: -10px;">' + tDict.getValueFor('PROMPT') + '</span>';
						_html += '</td>';
						_html += '</tr>';
						var mUUID3 = -1;
						for (j = 0; j < oChildren.length; j++) {
							...
							_html += '<input type="checkbox" class="sideMenuCB" id="cb_menuItem_' + mUUID3 + '" onclick="ClientMenuObj.onCheckBoxClicked(ClientMenuObj.instances[' + this.id + '], this.id, this.checked); return true;">&nbsp;' + '<span class="menuClass">' + tDict3.getValueFor('PROMPT') + '</span>';
							_html += '</td>';
							_html += '</tr>';
						//	_alert('\n\n' + _html);
						...
						_html += ((mUUID.trim().length > 0) ? '<input type="checkbox" class="sideMenuCB" id="cb_menuItem_' + mUUID + '" onclick="ClientMenuObj.onCheckBoxClicked(ClientMenuObj.instances[' + this.id + '], this.id, this.checked); return true;">&nbsp;' : '') + '<span class="menuClass">' + tDict.getValueFor('PROMPT') + '</span>';
					}
					_html += '</td>';
