	...
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<!--
			function queueUp_AJAX_Sessions3() {
				...
				pQueryString = '';
				oAJAXEngine.addNamedContext('#const_menuDisplayRecord_symbol#', pQueryString); // spanName is the name of the context... the GUI follows this same pattern...
				execJSAPI_func('getMenuEditorContents', 'displayClientMenuData(' + oAJAXEngine.js_global_varName + ')');
			}
					...
					if (!!qData) {
						...
						//	_alert(oParms);
							...
							if (!!cObj) {
								...
								global_menuEditorObj.onOpenAddMenuItem = function () {
									...
									_alertM('global_menuEditorObj.onOpenAddMenuItem() :: ');
								}
								
								global_menuEditorObj.onCloseMenuItemEditor = function () {
									var cObj = getGUIObjectInstanceById('div_contents_menuRecord');
									if (!!cObj) {
										cObj.disabled = false;
									}
									...
									_alertM('global_menuEditorObj.onCloseMenuItemEditor() :: ');
								}
								...
								global_menuEditorObj.onSaveMenuItem = function (brObj, epObj) {
									dispOrder = brObj.selectedIndex + 1;
									sPrompt = epObj.value;
									...
									//	_alert('aDict = [' + aDict + ']');
										if (this.isAddingMenuSubMenu) {
											execJSAPI_func('onCommitSubMenu', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), srcId, aDict.getValueFor('menuUUID'), aDict.getValueFor('parentId'), dispOrder);
											...
											execJSAPI_func('onCommitMenuItem', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), srcId, aDict.getValueFor('menuUUID'), aDict.getValueFor('parentId'), dispOrder);
								...
								global_menuEditorObj.onPasteFromClipboard = function (brObj, clipObj, bcpasteObj) {
									function cutItemsFromClipBoard(_MENUUUID, _PARENTID) {
										...
										if (selected_container != null) {
											...
											for (i = 0; i < selected_container.length; i++) {
												temp_Dict = DictionaryObj.getInstance(selected_container[i]);
												aMENUUUID = temp_Dict.getValueFor('MENUUUID');
												cutItemsFromClipBoard(aMENUUUID);
												DictionaryObj.removeInstance(temp_Dict.id);
											}
										}
									...
									execJSAPI_func('onPasteSubMenu', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', selected_MENUUUID, newDispOrder, ((currentParentSubMenu_id.trim().length == 0) ? '-1' : currentParentSubMenu_id));
									
								//	_alert('selected_MENUUUID = [' + selected_MENUUUID + ']' + ', newDispOrder = [' + newDispOrder + ']' + '\n\n' + 'coMenuDict = [' + coMenuDict + ']\n\n' + 'cuMenuDict = [' + cuMenuDict + ']');
									_alertM('global_menuEditorObj.onPasteMenuItem()');
								}
...
// +++ Debug this code to allow this function to bail-out whenever it runs out of things to find...
								...
								global_menuEditorObj.onDeleteMenuItem = function (brObj, bcutObj, bdelObj) {
									...
									if ( (!!oAR) && (oAR.length > 0) ) {
										execJSAPI_func('onDeleteMenuItem', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', oAR);
									}
								...
								global_menuEditorObj.onClickMenuItem = function (brObj, bdelObj) {
									...
									if (!!bdelObj) bdelObj.disabled = ((_isParent) ? true : false);
