	...
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<!--//
			...
			function init() {
				...
				oGateway.onReceive = function (){
					...
					if (this.isDebugMode()) alert('oGateway.mode = [' + oGateway.mode + ']' + '\n' + this.received + '\n' + 'oGateway.js_global_varName = [' + eval(oGateway.js_global_varName) + ']');
			...
			function window_onUnload() {
				...
				var js_Global = eval(oGateway.js_global_varName);
				try { js_Global = ((js_Global != null) ? object_destructor(js_Global) : null); } catch(e) {}																		// avoid closures by destroying the previous object before making a new one...
		...
		<!--
			function perform_click_btn_addMenuName() {
				...
				var cObj = getGUIObjectInstanceById('btn_addMenuName');
				if (cObj != null) {
					if (cObj.value == '*') {
				...
				if (eObj != null) {
					if (eObj.value.trim().length > 0) {
						if (bool_performing_btn_addMenuName) {
							execJSAPI_func('addTopLevelMenuName', 'displayTopLevelMenuNames(' + oGateway.js_global_varName + ')', eObj.value.trim());
						} else {
							execJSAPI_func('saveTopLevelMenuName', 'displayTopLevelMenuNames(' + oGateway.js_global_varName + ')', eObj.value.trim(), eObj.title.trim());
						}
					} else {
			...
			function perform_click_btn_dropMenuName(_objID_prefix, _iNum, _recID) {
				if (confirm('Are you sure you really want to delete that record from the database ?')) {
					...
					if (_recID > -1) {
						execJSAPI_func('dropTopLevelMenuName', 'displayTopLevelMenuNames(' + oGateway.js_global_varName + ')', _recID);
					} else {
			...
			function perform_click_btn_editMenuName(_objID_prefix, _iNum, _recID) {
				var _handle = global_guiActionsObj.push('btn_addMenuName');
				if (_handle > -1) {
					global_guiActionsObj.replaceAspectNamedFor(_handle, 'value', '*');
					global_guiActionsObj.replaceAspectNamedFor(_handle, 'title', 'Click to Save the edited Menu Name to the Db.');
				}
				var _handle2 = global_guiActionsObj.push('btn_revertMenuName');
				if (_handle2 > -1) {
					global_guiActionsObj.replaceStyleNamedFor(_handle2, 'display', 'display: ' + const_inline_style + ';');
				}
				
				var _handle3 = global_guiActionsObj.push('txt_menuName');
				if (_handle3 > -1) {
					...
					if (cObj != null) {
						...
						setFocusSafelyById('txt_menuName');
					}
				}
			}
		...
		<table width="100%" border="1" cellspacing="-1" cellpadding="-1" bgcolor="##FFFF80" style="margin-top: 20px;">
			<tr>
				<td align="left" valign="top">
					<button name="btn_getContents" id="btn_getContents" class="buttonMenuClass" onclick="execJSAPI_func('getTopLevelMenuNames', 'displayTopLevelMenuNames(' + oGateway.js_global_varName + ')'); return false;">[?]</button>
				</td>
				<td align="left" valign="top">
