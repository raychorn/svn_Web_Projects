...
// +++			
			function _onCommitMenuSomething(v) { // returns a Dictionary instance...
				...
				var srcTableName = '';
				if (ar.length > 2) {
					ar = global_dict.getValueFor(ar[2].URLDecode());
					srcId = ar[0];
					if (ar.length > 1) {
						srcTableName = ar[1];
					}
						...
						//	_alert(oParms);
							...
							if (!!cObj) {
								...
								global_menuEditorObj.onCommitSubMenu = function (v) {
									...
									execJSAPI_func('onCommitSubMenu', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), aDict.getValueFor('srcId'), aDict.getValueFor('menuUUID'), aDict.getValueFor('parentId'), aDict.getValueFor('dispOrder'));
									_alertM('global_menuEditorObj.onCommitSubMenu(v = [' + v + '])' + '\nglobal_dict = [' + global_dict + ']' + '\nar = [' + ar + ']' + ', srcId = [' + srcId + ']' + ', srcTableName = [' + srcTableName + ']' + ', menuUUID = [' + menuUUID + ']');
								}

								global_menuEditorObj.onCommitMenuItem = function (v) {
									var aDict = _onCommitMenuSomething(v);
									execJSAPI_func('onCommitMenuItem', 'displayMenuEditorData(' + oAJAXEngine.js_global_varName + ')', aDict.getValueFor('srcTableName'), aDict.getValueFor('srcId'), aDict.getValueFor('menuUUID'), aDict.getValueFor('parentId'), aDict.getValueFor('dispOrder'));
