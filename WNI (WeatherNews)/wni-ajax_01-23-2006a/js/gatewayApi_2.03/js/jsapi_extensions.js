...
//	var _db = '';
//	_db += '\n(1) _stack_jsapi_serialization.length = [' + _stack_jsapi_serialization.length + ']';
	_stack_jsapi_serialization.push(f);
//	_db += '\n(2) _stack_jsapi_serialization.length = [' + _stack_jsapi_serialization.length + ']';
//	alert('register_jsapi_function(' + f + ')' + _db);
}

function handle_next_jsapi_function() {
	var f = _stack_jsapi_serialization.pop();
//	alert('handle_next_jsapi_function() :: (' + f + ')');
	if (!!f) {
		eval(f);
	}
}
