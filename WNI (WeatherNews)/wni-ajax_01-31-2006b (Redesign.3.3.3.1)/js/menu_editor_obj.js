...
menuEditorObj._const_menu_copy_to_clipboard_symbol = 'menu_copy_to_clipboard';
menuEditorObj._const_menu_delete_item_symbol = 'menu_delete_item';
...
menuEditorObj.processClipboardPasteBecomesMove = function(anObj) {
	...
	var pmObj = getGUIObjectInstanceById(menuEditorObj.const_clipboard_paste_to_menu);
...
menuEditorObj.processClipboardRemoveSelected = function(clipObj, _menuEditorObj) {
	return ( ( (!!_menuEditorObj) && (!!clipObj) ) ? _menuEditorObj._processClipboardRemoveSelected(clipObj) : alert('ERROR: Programming Error - Undefined Objects _menuEditorObj is (' + _menuEditorObj + ') and clipObj is (' + clipObj + ') in function known as menuEditorObj.processClipboardRemoveSelected().'));
};
...
menuEditorObj.ProcessMenuSaveToDatabase = function(mcObj, _menuEditorObj) {
	return ( ( (!!_menuEditorObj) && (!!mcObj) ) ? _menuEditorObj._ProcessMenuSaveToDatabase(mcObj) : alert('ERROR: Programming Error - Undefined Objects _menuEditorObj is (' + _menuEditorObj + ') and mcObj is (' + mcObj + ') in function known as menuEditorObj.ProcessMenuSaveToDatabase().'));
};
...
menuEditorObj.refreshUrlType = function(_url, _menuEditorObj) {
	return ( ( (!!_menuEditorObj) && (!!_url) ) ? _menuEditorObj._refreshUrlType(_url) : alert('ERROR: Programming Error - Undefined Objects _menuEditorObj is (' + _menuEditorObj + ') and _url is (' + _url + ') in function known as menuEditorObj.refreshUrlType().'));
};

menuEditorObj._refreshUrlType = function(_menuEditorObj) {
	return ( (!!_menuEditorObj) ? _menuEditorObj.__refreshUrlType() : alert('ERROR: Programming Error - Undefined Objects _menuEditorObj is (' + _menuEditorObj + ') in function known as menuEditorObj._refreshUrlType().'));
};

menuEditorObj.processChangedUrlType = function(_menuEditorObj) {
	return ( (!!_menuEditorObj) ? _menuEditorObj._processChangedUrlType() : alert('ERROR: Programming Error - Undefined Objects _menuEditorObj is (' + _menuEditorObj + ') in function known as menuEditorObj.processChangedUrlType().'));
};
...
menuEditorObj.const_menu_item_editor_prompt_id = function(_menuEditorObj) {
	return ( (!!_menuEditorObj) ? _menuEditorObj.const_menu_item_editor_prompt_id : alert('ERROR: Programming Error - Undefined Objects _menuEditorObj is (' + _menuEditorObj + ') in function known as menuEditorObj.const_menu_item_editor_prompt_id().'));
};

menuEditorObj.defaultGUI = function(_menuEditorObj, _site_menu_text_color, _outerTableWidth) {
	var _html = '';
	
	if ( (_menuEditorObj == null) || (_menuEditorObj.id == null) ) { // cannot use (!!) to test these conditions because 0 is valid and !! tests for not zero as well as not null...
	...
	_html += '<input type="button" id="addit_menu_item" value="Add Menu Item" class="buttonClass" title="Click this button to add a menu item.  Menu items are used to link to internal pages of content or to external documents such as Panagons docs or to external sites.  Menu items are different than SubMenu Items in that a Menu Item provides a single link to something whereas a SubMenu provides a container for a number of Menu Items." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuItemAdditor(brObj, menuEditorObj.instances[' + _menuEditorObj.id + ']) } return false;">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '<tr>';
	...
	_html += '<input type="button" id="addit_menu_submenu" value="Add Empty\nSubMenu" class="buttonClass" title="Click this button to add a SubMenu Item.  SubMenu Items may contain one or more Menu Items and are used to extend the menu horizontally whereas Menu Items extend the Menu vertically." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuSubMenuAdditor(brObj, menuEditorObj.instances[' + _menuEditorObj.id + ']) } return false;">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '<tr>';
	...
	_html += '<input type="button" id="menu_browse_into" value="Open\nSub Menu" disabled class="buttonClass" title="Click this button to Open a SubMenu, the SubMenu that is highlighted in the list directly above this button.  The other way to Open a SubMenu is to double-click the SubMenu item." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuBrowseInto(brObj, menuEditorObj.instances[' + _menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '<td width="50%" align="right">';
	_html += '<input type="button" id="menu_browse_out" id="menu_browse_out" value="Close\nSub Menu" disabled class="buttonClass" title="Click this button to Close a SubMenu that was previously opened." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuBrowseOut(brObj, menuEditorObj.instances[' + _menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '</tr>';
	_html += '</table>';
	_html += '</td>';
	...
	_html += '</td>';
	_html += '<td align="center" valign="top">';
	_html += '<input type="button" id="menu_cut_to_clipboard" disabled value="Cut to\nClipboard" class="buttonClass" title="Click this button to Cut a Menu Item or SubMenu Item to the ClipBoard. This function allows you to reorder the Menu; once a Menu Item or SubMenu is on the ClipBoard it may be Moved back to the Menu in a different position or into a different SubMenu container." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuCutToClipboard(brObj, menuEditorObj.instances[' + _menuEditorObj.id + ']) } return false;">';
	_html += '<br><br>';
	_html += '<input type="button" id="menu_delete_item" disabled value="Delete\nItem" class="buttonClass" title="Click this button to delete a Menu Item from the Menu.  You may also delete a SubMenu after the SubMenu is empty.  Use this button to delete each Menu Item from the SubMenu and then close the SubMenu to use this button to delete the empty SubMenu." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuCutToThinAir(brObj, menuEditorObj.instances[' + _menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '</tr>';
	_html += '</table>';
	_html += '</td>';
...
menuEditorObj.prototype = {
	id : -1,
	bool_debugMode : false,     // when this boolean is true then perform debug processing, if any exists...
...
//	const_menu_copy_to_clipboard_symbol : menuEditorObj._const_menu_copy_to_clipboard_symbol, // this allows the copy to clipboard to be replaced by a delete item button without having to recode a lot of logic...
	const_menu_copy_to_clipboard_symbol : menuEditorObj._const_menu_delete_item_symbol,       // this allows the copy to clipboard to be replaced by a delete item button without having to recode a lot of logic...
	...
	const_menu_item_editor_prompt_id : '_menu_item_editor_prompt',
	...
	const_empty_symbol : '',
	bool_menuEditorPromptsDisabled : true, // users cannot edit the menu prompts directly because they reside in separate tables...
	guiHTML : '',
	_menu_in_browser : [],
	menuArray : [],
	_menu_stack : [],
		...
		function debugMenuNode(c, obj) {
				...
				if (!!c[i]) {
					if (typeof c[i] == const_object_symbol) {
						...
						if (ar[1] != id) {
							...
							if (obj.bool_debugMode) alert('A. c[' + i + '] = [' + c[i] + ']' + ', id = [' + id + ']' + ', c[i][0] = [' + c[i][0] + ']' + ', ar = [' + ar + ']' + ', ar[1] = [' + ar[1] + ']' + '\n' + 'obj._menu_navigation_stack = [' + obj._menu_navigation_stack + ']');
							obj._processMenuBrowseInto(brObj);
							if (obj.bool_debugMode) alert('B. c[' + i + '] = [' + c[i] + ']' + ', id = [' + id + ']' + ', c[i][0] = [' + c[i][0] + ']' + ', ar = [' + ar + ']' + ', ar[1] = [' + ar[1] + ']' + '\n' + 'obj._menu_navigation_stack = [' + obj._menu_navigation_stack + ']');
							debugMenuNode(c[i], obj);
						} else {
							foundIt = true;
							brObj.selectedIndex = i;
							obj._menu_navigation_stack.push(i);
							obj._processMenuBrowseInto(brObj);
							if (obj.bool_debugMode) alert('(***) c[' + i + '] = [' + c[i] + ']' + ', id = [' + id + ']' + ', c[i][0] = [' + c[i][0] + ']' + ', ar = [' + ar + ']' + ', ar[1] = [' + ar[1] + ']' + '\n' + 'obj._menu_navigation_stack = [' + obj._menu_navigation_stack + ']');
							break;
						}
					} else {
						_i++;
			...
			if ( (foundIt == false) && (obj._menu_navigation_stack.length > 0) ) {
				while (obj._menu_navigation_stack.length > 0) {
					if (obj.bool_debugMode) alert('C. ' + 'obj._menu_navigation_stack = [' + obj._menu_navigation_stack + ']' + ', brObj.selectedIndex = [' + brObj.selectedIndex + ']');
					obj._processMenuBrowseOut(brObj);
					brObj.selectedIndex = obj._menu_navigation_stack.pop();
					if (obj.bool_debugMode) alert('D. ' + 'obj._menu_navigation_stack = [' + obj._menu_navigation_stack + ']' + ', brObj.selectedIndex = [' + brObj.selectedIndex + ']');
				}
			}
			return;
		}
		if (!!brObj) {
			if (this.bool_debugMode) alert('(retrieveMenuByUUID) At the top of the menu ?' + ' this._menu_navigation_stack.length = [' + this._menu_navigation_stack.length + ']');
			return debugMenuNode(mm, this);
		}
	},
	navigateToUUID : function(id, bool_autoNavigate) { // assumption: we always begin at the top of the menu...
		var brObj = getGUIObjectInstanceById('menu_browser');
		if (!!brObj) {
			if (brObj.selectedIndex != 0) {
				brObj.selectedIndex = 0; // force the first item to be selected...
			}
			return this.retrieveMenuByUUID(brObj, id, this.menuArray, bool_autoNavigate);
		}
	},
	navigateBackFromUUID : function(brObj) { // assumption: we always begin within some subMenu somewhere...
		var brObj = ((!!brObj) ? brObj : getGUIObjectInstanceById('menu_browser'));
		if (!!brObj) {
			if (this.bool_debugMode) alert('(navigateBackFromUUID)-A At the top of the menu ?' + ' this._menu_navigation_stack.length = [' + this._menu_navigation_stack.length + ']');
			...
			if (this.bool_debugMode) alert('(navigateBackFromUUID)-B At the top of the menu ?' + ' this._menu_navigation_stack.length = [' + this._menu_navigation_stack.length + ']');
		}
	},
			...
			for (var i = (this._menu_stack.length == 0) ? 0 : 1; i < mm.length; i++) {
				if (!!mm[i]) {
					...
					if (_prompt.length > 0) {
						...
						ar.push(typeof mm[i]);
						ar.push(_uuid);
						oObj = new Option( _prompt, ar.join('|')); // (+++) this is the central point-of-focus for all actions on the menu itself...
	...
	_pasteItemToMenu : function(v, isAfter) {
		...
		if (!!brObj) {
			...
			if (!!c) {
				...
				if (typeof v == const_object_symbol) {
					this.onPasteSubMenu(v, offset - 1);
				} else {
					this.onPasteMenuItem(v, offset - 1);
				}
				this.rebuildMenu(this.menuArray, false);
			}
		}
	...
	initMenuEditorClipboardObjects : function() {
		...
		if (!!rObj) {
			...
			html += '<tr>';
			html += '<td>';
			html += '<input type="button" id="clipboard_remove_item" value="Remove Clipboard Item" disabled class="buttonClass" onClick="var clipObj = getGUIObjectInstanceById(\'menu_clipboard\'); if (!!clipObj) { menuEditorObj.processClipboardRemoveSelected(clipObj, menuEditorObj.instances[' + this.id + ']) } return false;">';
			html += '</td>';
			html += '</tr>';
			...
			html += '<tr>';
			html += '<td>';
			var _html = _setup_tooltip_handlers(menuEditorObj.const_clipboard_paste_to_menu, this.const_empty_symbol);
		...
		if (!!eObj) {
			...
			if ( (isObjValidHTMLValueHolder(pListObj)) && (isObjValidHTMLValueHolder(pnListObj)) ) {
				...
				html += '<tr>';
				html += '<td align="left">';
				html += '<font size="1"><small><b>Prompt:</b></small></font>&nbsp;<input type="text" id="' + this.const_menu_item_editor_prompt_id + '" size="39" maxlength="50" class="textEntryClass" onkeypress="return false;" onchange="return false;" onkeydown="return false;" onkeyup="return false;">';
				html += '</td>';
				html += '</tr>';
				...
				html += '<select id="menu_item_editor_pageList" size="4" style="font-size: 10px; line-height: 10px;" onchange="menuEditorObj.refreshMenuItemEditorSaveButton(menuEditorObj.instances[' + this.id + ']); return true;">';
				html += '</select>';
				html += '</div>';
				...
				html += '<select ' + _html + ' size="1" style="font-size: 10px; line-height: 10px;" onchange="menuEditorObj.processChangedUrlType(menuEditorObj.instances[' + this.id + ']); return true;">';
				html += '</select>';
				html += '</td>';
				html += '</tr>';
				html += '<tr>';
				...
				html += '<tr>';
				html += '<td>';
				html += '<textarea cols="35" rows="5" name="menu_item_editor_http" wrap="soft" style="font-size: 10px; line-height: 10px;" onkeyup="menuEditorObj._refreshUrlType(menuEditorObj.instances[' + this.id + ']); menuEditorObj.refreshMenuItemEditorSaveButton(menuEditorObj.instances[' + this.id + ']); return true;"></textarea>';
				html += '</td>';
				html += '</tr>';
				html += '</table>';
				html += '</td>';
