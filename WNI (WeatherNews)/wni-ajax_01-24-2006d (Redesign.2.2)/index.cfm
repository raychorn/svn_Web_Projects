...
<cfset _menu_editor_core_obj_js = "#_js_path#menu_editor_core_obj.js">
...
<cfoutput>
	...
	<head>
		...
		<script language="JavaScript1.2" src="#_menu_editor_core_obj_js#" type="text/javascript"></script>
	...
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<!--
			function queueUp_AJAX_Sessions() {
				...
				jsCode += "oGateway.addNamedContext('#const_layerGroupRecord_symbol#', pQueryString); ";
				jsCode += "global_maxRecsPerPage['#const_layerGroupRecord_symbol#'] = 10; ";
				jsCode += "execJSAPI_func('getLayerGroupNames', 'displayLayerGroupNames(" + oGateway.js_global_varName + ")'); ";
...
// +++
			...
			function perform_click_btn_addMenuName(_spanName, _cfm_add_cmd, _cfm_save_cmd) {
				...
				if (dataVals.length > 0) {
					if (bool_performing_btn_addMenuName) {
						execJSAPI_func(_cfm_add_cmd + '_v', 'displayTopLevelMenuNames(' + oGateway.js_global_varName + ')', dataVals.length, dataVals);
					} else {
						execJSAPI_func(_cfm_save_cmd + '_v', 'displayTopLevelMenuNames(' + oGateway.js_global_varName + ')', dataVals.length, dataVals);
					}
				} else {
...
// +++
		...
		<input type="hidden" id="_GetCurrentContent_pageList" value="">
		<input type="hidden" id="_GetCurrentContent_notLinkables" value="">
												...
												<table cellpadding="-1" cellspacing="-1">
													...
													<tr>
														<td>
															<div id="_menu_submenu_additor_button" style="display: inline;">
																<cfset _tooltips_ = "id=addit_menu_submenu">
																<input type="button" #_tooltips_# value="Add Empty#Request.const_Cr#SubMenu" style="font-size: 9px;" title="Click this button to add a SubMenu Item.  SubMenu Items may contain one or more Menu Items and are used to extend the menu horizontally whereas Menu Items extend the Menu vertically." onClick="var brObj = getGUIObjectInstanceById('menu_browser'); if (brObj != null) { menuEditorCoreObj.processMenuSubMenuAdditor(brObj, menuEditorCoreObj.instances[global_menuEditorObj.id]) } return false;">
															</div>
														</td>
													</tr>
													<tr>
													...
													<tr>
														<td id="hilite_color_preview2" bgcolor="#_site_menu_text_color#">
															<div id="_menu_colors_button2" style="display: none;">
																<cfset _tooltips_ = ""> <!--- id=change_menu_colors2 --->
																<input type="button" #_tooltips_# value="Menu#Request.const_Cr#Text Color" style="font-size: 9px; display: none;" title="Click this button to change the menu's text color.  This is the color of the text and not the menu color." onClick="var brObj = getGUIObjectInstanceById('menu_browser'); if (brObj != null) { processMenuChangeTextColors(brObj) } return false;">
								...
								<td width="50%" valign="top" align="right">
									<table cellpadding="-1" cellspacing="-1">
										<tr>
															...
															<table width="100%" cellpadding="-1" cellspacing="-1">
																<tr>
																	<td width="50%" align="left">
																		<cfset _tooltips_ = "id=menu_browse_into">
																		<input type="button" #_tooltips_# value="Open#Request.const_Cr#Sub Menu" disabled style="font-size: 9px;" title="Click this button to Open a SubMenu, the SubMenu that is highlighted in the list directly above this button.  The other way to Open a SubMenu is to double-click the SubMenu item." onClick="var brObj = getGUIObjectInstanceById('menu_browser'); if (brObj != null) { menuEditorCoreObj.processMenuBrowseInto(brObj, menuEditorCoreObj.instances[global_menuEditorObj.id]) } return false;">
																	</td>
																	<td width="50%" align="right">
																		<cfset _tooltips_ = "id=menu_browse_out">
																		<input type="button" #_tooltips_# id="menu_browse_out" value="Close#Request.const_Cr#Sub Menu" disabled style="font-size: 9px;" title="Click this button to Close a SubMenu that was previously opened." onClick="var brObj = getGUIObjectInstanceById('menu_browser'); if (brObj != null) { menuEditorCoreObj.processMenuBrowseOut(brObj, menuEditorCoreObj.instances[global_menuEditorObj.id]) } return false;">
																	</td>
																</tr>
															</table>
														</td>
											...
											<td align="center" valign="top">
												<cfset _tooltips_ = "id=menu_cut_to_clipboard">
												<input type="button" #_tooltips_# disabled value="Cut to#Request.const_Cr#Clipboard" style="font-size: 9px;" title="Click this button to Cut a Menu Item or SubMenu Item to the ClipBoard. This function allows you to reorder the Menu; once a Menu Item or SubMenu is on the ClipBoard it may be Moved back to the Menu in a different position or into a different SubMenu container." onClick="var brObj = getGUIObjectInstanceById('menu_browser'); if (brObj != null) { menuEditorCoreObj.processMenuCutToClipboard(brObj, global_menuEditorObj) } return false;">
												<br><br>
												<cfset _tooltips_ = "id=menu_delete_item">
												<input type="button" #_tooltips_# disabled value="Delete#Request.const_Cr#Item" style="font-size: 9px;" title="Click this button to delete a Menu Item from the Menu.  You may also delete a SubMenu after the SubMenu is empty.  Use this button to delete each Menu Item from the SubMenu and then close the SubMenu to use this button to delete the empty SubMenu." onClick="var brObj = getGUIObjectInstanceById('menu_browser'); if (brObj != null) { menuEditorCoreObj.processMenuCutToThinAir(brObj, global_menuEditorObj) } return false;">
											</td>
										</tr>
									</table>
								</td>
		...
		<cfset _isLocal = Trim(Request.CommonCode.isServerLocal())>

		<script language="JavaScript1.2" type="text/javascript">
		<!--
			global_menuEditorObj = menuEditorCoreObj.getInstance('div_menu_browser', 'menu_browser');
			
			_isServerLocal_ = '#_isLocal#';
			
			b_isServerLocal = ((_isServerLocal_.toUpperCase() == 'YES') ? true : false);
			if (b_isServerLocal) {
				global_menuEditorObj._global_menu_mode = b_isServerLocal; // true for the new menu, false for the original menu...
			}
		// -->
		</script>
