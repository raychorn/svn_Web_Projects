...
//	_alert('(processXmlHttpRequestStateChange) :: AJAXEngine._xmlHttp_reqObject.oRequest = [' + AJAXEngine._xmlHttp_reqObject.oRequest + ']');
//	_alert('(processXmlHttpRequestStateChange) :: AJAXEngine._xmlHttp_reqObject.oRequest.readyState = [' + AJAXEngine._xmlHttp_reqObject.oRequest.readyState + ']');
	    if ( (!!AJAXEngine._xmlHttp_reqObject.oRequest) && (AJAXEngine._xmlHttp_reqObject.oRequest.readyState == 4) ) {
	        try {
//	_alert('(processXmlHttpRequestStateChange) :: AJAXEngine._xmlHttp_reqObject.oRequest.status = [' + AJAXEngine._xmlHttp_reqObject.oRequest.status + ']' + ', bool = [' + (AJAXEngine._xmlHttp_reqObject.oRequest.status && AJAXEngine._xmlHttp_reqObject.oRequest.status == 200) + ']');
...
//	_alert('(processXmlHttpRequestStateChange) :: A. response');
				 	response = response.stripCrLfs();
//	_alert('(processXmlHttpRequestStateChange) :: B. response');
...
//	_alert('(processXmlHttpRequestStateChange) :: B.1 ' + 'bof_f = [' + bof_f + ']' + ', eof_f = [' + eof_f + ']');
					if (eof_f > 0) {
						eof_f += AJAXEngine.eof_cfajax_comment_symbol.length; 
					}
//	_alert('(processXmlHttpRequestStateChange) :: B.2 ' + 'bof_f = [' + bof_f + ']' + ', eof_f = [' + eof_f + ']');
					if (bof_f > 0) {
						response = response.substring(Math.min(bof_f, eof_f),Math.max(bof_f, eof_f));
					}
//	_alert('(processXmlHttpRequestStateChange) :: B.3 ' + 'bof_f = [' + bof_f + ']' + ', eof_f = [' + eof_f + ']');
//	_alert('(processXmlHttpRequestStateChange) :: C. response');
				...
				_alert(jsErrorExplainer(ex, '(processXmlHttpRequestStateChange)'));
	        }
		}
	},
...
menuEditorObj.prototype = {
	...
	_pasteItemToMenu : function(v, isAfter) {
		...
		if (!!brObj) {
			...
			if (this.debugMode) alert('A. v = [' + v + '], offset = [' + offset + ']');
			try {
				var c = this.performMenuPasteAtLevel(v, offset);
			} catch(e) {
				jsErrorExplainer(e, '333 ' + 'v = [' + v + '], offset = [' + offset + ']', true);
			}
			if (this.debugMode) alert('B. v = [' + v + '], offset = [' + offset + ']' + ', c = [' + c + ']');
			if (this.isAutoLoadingMenu) return;
			if (!!c) {
			//	this.populateSelectWithMenu(brObj, c);
			if (this.debugMode) alert('C. v = [' + v + '], offset = [' + offset + ']' + ', c = [' + c + ']');
				if (isAfter == false) {
	...
	retrieveMenuAtLevel : function(mm, stack, i) {
		...
		if (typeof c == 'string') {
		//	c = mm;
			alert(911 + ', mm = [' + mm + ']' + '\n' + 'i = [' + i + ']' + '\n' + 'c = [' + c + ']' + '\n' + 'stack = [' + stack + ']' + '\n' + 'this.menuArray = [' + this.menuArray + ']' + '\n' + 'this._menu_stack = [' + this._menu_stack + ']' + '\n' + this.debugMenu(this.menuArray));
		}
...
ClientMenuObj.prototype = {
	...
	//	_alert('this.coMenuDict = [' + this.coMenuDict + ']' + ', this.cuMenuDict = [' + this.cuMenuDict + ']');
		iAR = this.coMenuDict.getValueFor(this.aMENUUUID);
	//	_alert('\n\iAR.length = [' + iAR.length + ']' + ', (typeof iAR) = [' + (typeof iAR) + ']' + ', iAR = [' + iAR + ']');
