					...
					if (!!qData) {
						...
						//	_alert(oParms);
							...
							if (!!cObj) {
								...
								global_menuEditorObj = menuEditorCoreObj.getInstance('div_menu_browser', 'menu_browser', '#_site_menu_text_color#', 400);
								_html += global_menuEditorObj.getGUIcontainer('div_menuEditor_container');
								
								global_menuEditorObj.onCommitSubMenu = function (v) {
									alert('global_menuEditorObj.onCommitSubMenu(v = [' + v + '])');
								}
					
								global_menuEditorObj.onCommitMenuItem = function (v) {
									alert('global_menuEditorObj.onCommitMenuItem(v = [' + v + '])');
								}
								
								global_menuEditorObj.onChangeMenuItem = function (v) {
									alert('global_menuEditorObj.onChangeMenuItem(v = [' + v + '])');
								}
								
								global_menuEditorObj.onChangeSubMenu = function (v) {
									alert('global_menuEditorObj.onChangeSubMenu(v = [' + v + '])');
								}
					
								global_menuEditorObj.onOpenSubMenu = function (v) {
									alert('global_menuEditorObj.onOpenSubMenu(v = [' + v + '])');
								}
								
								global_menuEditorObj.onCloseSubMenu = function (v) {
									alert('global_menuEditorObj.onCloseSubMenu(v = [' + v + '])');
								}
								
								global_menuEditorObj.onCutToClipboardMenuItem = function (v) {
									alert('global_menuEditorObj.onCutToClipboardMenuItem(v = [' + v + '])');
								}
					
								global_menuEditorObj.onCutToClipboardSubMenu = function (v) {
									alert('global_menuEditorObj.onCutToClipboardSubMenu(v = [' + v + '])');
								}
								
								global_menuEditorObj.onDeleteMenuItem = function (v) {
									alert('global_menuEditorObj.onDeleteMenuItem(v = [' + v + '])');
								}
					
								global_menuEditorObj.rebuildMenu = function (mm, includeLinks) {
									alert('global_menuEditorObj.rebuildMenu(mm = [' + mm + '], includeLinks = [' + includeLinks + '])');
								}
