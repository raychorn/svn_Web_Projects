...
//_alert('\n\naDict = [' + aDict + ']');
			...
			function handleMenuOptionsClicks(brObj, id) {
				var sel = ((!!brObj) ? brObj.selectedIndex : -1);
				if (id == 'cb_menu_options_useRB') {
					alert('handleMenuOptionsClicks(brObj = [' + brObj + '], id = [' + id + '])' + ', sel = [' + sel + ']');
				}
			}
							...
							if (!bool_isAnyErrorRecords) {
								...
								if (!!cObj) {
									...
									global_menuEditorObj.onPopulateOptionsGUI = function () {
										...
										html += '<input type="checkbox" id="cb_menu_options_useRB" class="buttonClass" onclick="handleMenuOptionsClicks(menuEditorObj.brObj(global_menuEditorObj), this.id); return true;">&nbsp;';
