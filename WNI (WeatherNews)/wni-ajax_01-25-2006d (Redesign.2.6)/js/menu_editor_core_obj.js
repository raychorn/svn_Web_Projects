...
menuEditorCoreObj = function(id, div_menuBrowserID, menuBrowserID, _menu_bgColor, _menu_textColor, _outerWidth) {
	...
	this.guiHTML = menuEditorCoreObj.defaultGUI(this, _menu_bgColor, _menu_textColor, _outerWidth); 	// create the default GUI - this default GUI can be overridden with another that specifies the same DOM Object ID's...
};

menuEditorCoreObj.instances = [];

menuEditorCoreObj.getInstance = function(_div_menuBrowserID, _menuBrowserID, menuBgColor, menuTextColor, outerWidth) {
	...
	if(instance == null) {
		instance = menuEditorCoreObj.instances[menuEditorCoreObj.instances.length] = new menuEditorCoreObj(menuEditorCoreObj.instances.length, _div_menuBrowserID, _menuBrowserID, menuBgColor, menuTextColor, outerWidth);
	}
	return instance;
};
...
menuEditorCoreObj.defaultGUI = function(menuEditorObj, _site_menu_background_color, _site_menu_text_color, _outerTableWidth) {
	var _html = '';
	
	if ( (menuEditorObj == null) || (menuEditorObj.id == null) ) {
		alert('ERROR: Programming Error - Missing menuEditorObj (' + menuEditorObj + ') menuEditorObj.id = [' + menuEditorObj.id + '] in menuEditorCoreObj.defaultGUI().');
		return;
	}

	_site_menu_background_color = ((!!_site_menu_background_color) ? _site_menu_background_color : '');
	...
	_html += '<tr>';
	_html += '<td>';
	_html += '<div id="_menu_item_editor" style="display: none;">';
	_html += '</div>';
	_html += '</td>';
	_html += '<td>';
	...
	_html += '<input type="button" id="addit_menu_item" value="Add Menu Item" style="font-size: 9px;" title="Click this button to add a menu item.  Menu items are used to link to internal pages of content or to external documents such as Panagons docs or to external sites.  Menu items are different than SubMenu Items in that a Menu Item provides a single link to something whereas a SubMenu provides a container for a number of Menu Items." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorCoreObj.processMenuItemAdditor(brObj, menuEditorCoreObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '<tr>';
	...
	_html += '<input type="button" id="addit_menu_submenu" value="Add Empty\nSubMenu" style="font-size: 9px;" title="Click this button to add a SubMenu Item.  SubMenu Items may contain one or more Menu Items and are used to extend the menu horizontally whereas Menu Items extend the Menu vertically." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorCoreObj.processMenuSubMenuAdditor(brObj, menuEditorCoreObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '<tr>';
	...
	_html += '</tr>';
	_html += '<tr>';
	_html += '<td id="hilite_color_preview" bgcolor="' + _site_menu_background_color + '" style="display: none;">';
	_html += '<div id="_menu_colors_button" style="display: inline;">';
	_html += '<input type="button" ' + ((_site_menu_background_color.length > 0) ? 'id="change_menu_colors"' : '') + ' value="Menu Color" style="font-size: 9px; display: none;" title="Click this button to change the menu\'s background color.  This is the color of the menu and not the text color." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { processMenuChangeColors(brObj) } return false;">';
	_html += '<input type="hidden" id="hilite_color" value="' + _site_menu_background_color + '">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '<tr>';
	...
	_html += '</tr>';
	_html += '<tr>';
	_html += '<td id="hilite_color_preview2" bgcolor="' + _site_menu_text_color + '">';
	_html += '<div id="_menu_colors_button2" style="display: none;">';
	_html += '<input type="button" id="change_menu_colors2" value="Menu\nText Color" style="font-size: 9px; display: none;" title="Click this button to change the menu\'s text color.  This is the color of the text and not the menu color." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { processMenuChangeTextColors(brObj) } return false;">';
	_html += '<input type="hidden" id="hilite_color2" value="' + _site_menu_text_color + '">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '</table>';
	...
	_html += '<input type="button" id="menu_browse_into" value="Open\nSub Menu" disabled style="font-size: 9px;" title="Click this button to Open a SubMenu, the SubMenu that is highlighted in the list directly above this button.  The other way to Open a SubMenu is to double-click the SubMenu item." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorCoreObj.processMenuBrowseInto(brObj, menuEditorCoreObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '<td width="50%" align="right">';
	_html += '<input type="button" id="menu_browse_out" id="menu_browse_out" value="Close\nSub Menu" disabled style="font-size: 9px;" title="Click this button to Close a SubMenu that was previously opened." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorCoreObj.processMenuBrowseOut(brObj, menuEditorCoreObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '</tr>';
	_html += '</table>';
	_html += '</td>';
	...
	_html += '</td>';
	_html += '<td align="center" valign="top">';
	_html += '<input type="button" id="menu_cut_to_clipboard" disabled value="Cut to\nClipboard" style="font-size: 9px;" title="Click this button to Cut a Menu Item or SubMenu Item to the ClipBoard. This function allows you to reorder the Menu; once a Menu Item or SubMenu is on the ClipBoard it may be Moved back to the Menu in a different position or into a different SubMenu container." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorCoreObj.processMenuCutToClipboard(brObj, menuEditorCoreObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '<br><br>';
	_html += '<input type="button" id="menu_delete_item" disabled value="Delete\nItem" style="font-size: 9px;" title="Click this button to delete a Menu Item from the Menu.  You may also delete a SubMenu after the SubMenu is empty.  Use this button to delete each Menu Item from the SubMenu and then close the SubMenu to use this button to delete the empty SubMenu." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorCoreObj.processMenuCutToThinAir(brObj, menuEditorCoreObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '</tr>';
	_html += '</table>';
	_html += '</td>';
