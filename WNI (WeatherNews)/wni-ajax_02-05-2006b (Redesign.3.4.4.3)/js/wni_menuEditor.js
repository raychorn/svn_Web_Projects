...
menuEditorObj.defaultGUI = function(_menuEditorObj, _site_menu_text_color, _outerTableWidth) {
	...
	_html += '<table width="' + _outerTableWidth + '" border="0" cellpadding="-1" cellspacing="-1">' + '<tr>' + '<td align="left" valign="top">' + '<div>';
	_html += '<table width="*" cellpadding="-1" cellspacing="-1">' + '<tr>' + '<td width="*" align="right" valign="top">';
	_html += '<table width="100%" cellpadding="-1" cellspacing="-1">' + '<tr>' + '<td>';
	...
	_html += '</div>' + '</td>' + '</tr>' + '<tr>' + '<td>' + '&nbsp;' + '</td>' + '</tr>' + '<tr>' + '<td>';
	_html += '<div id="div_menuItemAdder" style="display: inline;">';
	_html += '<input type="button" id="btn_addMenuItem" value="Add Menu Item" class="buttonClass" title="Click this button to add a menu item." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { brObj.selectedIndex = -1; menuEditorObj.processMenuItemAdditor(brObj, menuEditorObj.instances[' + _menuEditorObj.id + ']) } return false;">';
	_html += '</div>' + '</td>' + '</tr>' + '<tr>' + '<td>' + '&nbsp;' + '</td>' + '</tr>' + '<tr>' + '<td>';
	...
	_html += '</div>' + '</td>' + '</tr>' + '<tr>' + '<td>' + '&nbsp;' + '</td>' + '</tr>' + '<tr>' + '<td>' + '&nbsp;';
	_html += '</td>' + '</tr>' + '</table>' + '</td>' + '</tr>' + '</table>' + '</td>';
	_html += '<td width="*" valign="top" align="right">';
	_html += '<table cellpadding="-1" cellspacing="-1">' + '<tr>' + '<td align="center" valign="top">';
	_html += '<table width="100%" cellpadding="-1" cellspacing="-1">' + '<tr>' + '<td bgcolor="#FFFF80">';
	_html += '<div id="div_menuEditor" style="display: inline;"></div>' + '</td>' + '</tr>' + '<tr>' + '<td>';
	_html += '<table width="100%" cellpadding="-1" cellspacing="-1">';
	_html += '<tr>' + '<td width="50%" align="left">';
	...
	_html += '</td>' + '<td width="50%" align="right">';
	...
	_html += '</td>' + '</tr>' + '</table>' + '</td>' + '</tr>' + '</table>' + '</div>' + '</td>';
	_html += '<td align="left" valign="top">' + '<div id="div_clipboard_wrapper"></div>' + '</td>' + '</tr>';
	_html += '<tr>' + '<td>' + '<div id="div_menuItemEditor" style="display: none;"></div>' + '</td>' + '</tr>' + '</table>';
...
menuEditorObj.prototype = {
	...
	onOpenAddMenuItem : function() {
		return -1; 
	},
	onOpenAddSubMenu : function() {
		return -1; 
	},
	onCloseMenuItemEditor : function() {
		return -1; 
	},
	...
	initMenuEditorClipboardObjects : function() {
		...
		if (!!rObj) {
			var html = '';
			html += '<div id="right_side_content">' + '<div id="welcome">' + '</div>';
			html += '<table cellpadding="-1" cellspacing="-1">' + '<tr>' + '<td>';
			html += '<table cellpadding="-1" cellspacing="-1">' + '<tr>' + '<td>' + '<center>';
			...
			html += '</select>' + '</center>' + '</td>' + '</tr>' + '<tr>' + '<td>';
			html += '<table cellpadding="-1" cellspacing="-1">' + '<tr>' + '<td>';
			...
			html += '</td>' + '</tr>' + '</table>' + '</td>' + '</tr>' + '</table>' + '</td>' + '</tr>' + '</table>' + '</div>';
		...
		if (!!eObj) {
			...
			html += '<form>' + '<table cellpadding="-1" cellspacing="-1">' + '<tr bgcolor="#c0c0c0">';
			html += '<td align="center">' + '<div id="_menu_item_editor_title">' + '<small><b>Edit Menu Item</b></small>';
			html += '</div>' + '</td>' + '</tr>' + '<tr>' + '<td align="left">';
...
ClientMenuObj.openClientSubMenuByUUID = function(oObj, aUUID, uuids) {
	try {
		...
		alert('Error :: Programming Error - ClientMenuObj.openClientSubMenuByUUID(oObj = [' + oObj + '], aUUID = [' + aUUID + '], uuids = [' + uuids + ']) ' + ((!!jsErrorExplainer) ? '\nReason: (' + jsErrorExplainer(e, '') + ')' : '') + '.');
	} finally {
	}
};
	...
	openClientSubMenuByUUID : function(aUUID, uuids) {
		...
		if (!!dObj) {
			if (dObj.innerHTML.length > 0) {
				...
				_html = '<table width="156" cellpadding="-1" cellspacing="-1">';
				for (i = 0; i < iAR.length; i++) {
					_html += '<tr class="sideMenuBG3">';
					tDict = DictionaryObj.getInstance(iAR[i]);
					mUUID = tDict.getValueFor('MENUUUID');
					_html += '<td>' + ((mUUID.trim().length > 0) ? '<input type="checkbox" id="cb_menuItem_' + mUUID + '" onchange="alert(this.value); return false;">&nbsp;' : '') + '<span class="menuClass">' + tDict.getValueFor('PROMPT') + '</span>' + '</td>';
