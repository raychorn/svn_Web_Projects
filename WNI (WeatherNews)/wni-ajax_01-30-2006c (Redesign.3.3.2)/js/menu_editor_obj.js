...
//	const_menu_copy_to_clipboard_symbol : menuEditorObj._const_menu_copy_to_clipboard_symbol, // this allows the copy to clipboard to be replaced by a delete item button without having to recode a lot of logic...
	...
	menuSelectItemByPosition : function(pos, id) {
		var _db = '';
		...
		if (!!brObj) {
			...
			_db += '\nsel = [' + sel + ']';
			if (sel > -1) {
				var opt = brObj.options[sel];
				_db += '\nopt = [' + opt + ']';
				if (!!opt) {
					ar = opt.value.split('|');
					_db += '\nopt.value = [' + opt.value + ']' + ', ar.length = [' + ar.length + ']' + ', ar = [' + ar + ']' + ', ar[1] = [' + ar[1] + ']' + ', id = [' + id + ']';
					if (ar[1] == id) {
						this._menu_navigation_stack.push(pos);
	...
	retrieveMenuByUUID : function(mm, id, bool_autoNavigate) {
		var i = -1;
		var toks = -1;
		bool_autoNavigate = ((bool_autoNavigate == true) ? bool_autoNavigate : false);
		for (i = 0; i < mm.length; i++) {
			if (typeof mm[i] == const_object_symbol) {
				if (bool_autoNavigate) this.menuSelectItemByPosition(i, id);
				return this.retrieveMenuByUUID(mm[i], id);
			} else {
				toks = mm[i].split('|');
				if ( (toks.length > 2) && (toks[1].trim() == id) ) {
					return mm;
				}
			}
		}
	...
	_retrieveMenuByUUID : function(id) { // assumption: we always begin at the top of the menu...
		return this.retrieveMenuByUUID(this.menuArray, id, false);
	},
	navigateToUUID : function(id, bool_autoNavigate) { // assumption: we always begin at the top of the menu...
		var c = this.retrieveMenuByUUID(this.menuArray, id, bool_autoNavigate);
	},
	navigateBackFromUUID : function(brObj) { // assumption: we always begin within some subMenu somewhere...
		...
		if (!!brObj) {
			while (this._menu_navigation_stack.length > 0) {
				...
				this._menu_navigation_stack.pop();
			}
		}
	//	alert('navigateToUUID(id = [' + id + ']) :: ' + ' id = [' + id + ']' + ', c = [' + c + ']' + '\nthis._menu_navigation_stack = [' + this._menu_navigation_stack + ']');
	},
	...
	processSelectedmenuItem : function(brObj) {
		...
		if ( (!!brObj) && (!!bbutObj) && (!!bcutObj) && (!!bcopyObj) && (!!edmiObj) && (!!abObj) && (!!asObj) ) {
			...
			//	alert('s = [' + s + ']' + ', o = [' + o + ']' + ', o.length = [' + o.length + ']');
	...
	initMenuEditorClipboardObjects : function() {
		...
		if (!!eObj) {
			...
			if ( (isObjValidHTMLValueHolder(pListObj)) && (isObjValidHTMLValueHolder(pnListObj)) ) {
				...
				html += '<tr>';
				html += '<td align="left">';
				html += '<font size="1"><small><b>Prompt:</b></small></font>&nbsp;<input type="text" id="_menu_item_editor_prompt" size="39" maxlength="50" class="textEntryClass" onkeyup="menuEditorObj.refreshMenuItemEditorSaveButton(menuEditorObj.instances[' + this.id + ']); return true;">';
				html += '</td>';
				html += '</tr>';
