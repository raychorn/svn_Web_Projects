		...
		case 'onDeleteMenuItem':
			if (Request.qryStruct.argCnt gt 0) {
				...
				if (NOT Request.dbError) {
					...
					if (NOT Request.dbError) {
//Request.cf_dump(Request.qOnDeleteReorderSiblings, 'Request.qOnDeleteReorderSiblings _sql_statement = [#_sql_statement#]', false);
						_sql_statement = "";
...
//Request.cf_dump(Request.qOnDeleteUpdateSiblings, 'Request.qOnDeleteUpdateSiblings _sql_statement = [#_sql_statement#]', false);
						if (NOT Request.dbError) {
...
//writeOutput('<b>Request.qLookupParentsKids.id[#i#] = [#Request.qLookupParentsKids.id[i]#]</b> ');
//writeOutput('<b>self_id = [#self_id#]</b> ');
//writeOutput('<b>k = [#k#]</b><br>');
									if (Request.qLookupParentsKids.id[i] neq self_id) {
										_sql_statement = _sql_statement & "UPDATE AvnMenuEditor SET dispOrder = #((i + k) - 1)# WHERE (id = #Request.qLookupParentsKids.id[i]#); ";
									} else {
										_sql_statement = _sql_statement & "UPDATE AvnMenuEditor SET dispOrder = #newDispOrder# WHERE (id = #Request.qLookupParentsKids.id[i]#); ";
										k = k + 1;
									}
								}
							}
...
//Request.cf_dump(Request.qLookupSiblingDispOrderCollisions, 'Request.qLookupSiblingDispOrderCollisions newDispOrder = [#newDispOrder#], [#_sql_statement#]', false);
...
//Request.cf_dump(Request.qOnCommitMenuItem, 'Request.qOnCommitMenuItem', false);
...
//Request.cf_dump(Request.qOnCommitReorderSiblings, 'Request.qOnCommitReorderSiblings [#_sql_statement#]', false);
					if (NOT Request.dbError) {
						_sql_statement = "";
...
//Request.cf_dump(Request.qOnCommitUpdateSiblings, 'Request.qOnCommitUpdateSiblings [#_sql_statement#]', false);
