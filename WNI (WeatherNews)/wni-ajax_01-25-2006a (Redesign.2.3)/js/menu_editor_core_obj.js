...
menuEditorCoreObj = function(id, div_menuBrowserID, menuBrowserID) {
	...
	this.guiHTML = menuEditorCoreObj.defaultGUI(this); 	// create the default GUI - this default GUI can be overridden with another that specifies the same DOM Object ID's...
...
menuEditorCoreObj.defaultGUI = function(menuEditorObj, _site_menu_background_color, _site_menu_text_color) {
	var _html = '';
	...
	_html += '<tr>';
	_html += '<td width="50%" align="right" valign="top">';
	...
	_html += '</table>';
	_html += '</td>';
	_html += '<td width="50%" valign="top" align="right">';
	_html += '<table cellpadding="-1" cellspacing="-1">';
	_html += '<tr>';
...
//	const_menu_copy_to_clipboard_symbol : menuEditorCoreObj._const_menu_copy_to_clipboard_symbol, // this allows the copy to clipboard to be replaced by a delete item button without having to recode a lot of logic...
	...
	_handle_menuBrowser_change : function() {
		var brObj = getGUIObjectInstanceById('menu_browser'); 
	//	alert('_handle_menuBrowser_change()' + ', brObj = [' + brObj + ']');
		if (!!brObj) { 
			this.processSelectedmenuItem(brObj);
		} 
