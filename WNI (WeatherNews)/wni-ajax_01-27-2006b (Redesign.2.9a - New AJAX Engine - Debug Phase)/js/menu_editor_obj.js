...
menuEditorObj.defaultGUI = function(menuEditorObj, _site_menu_text_color, _outerTableWidth) {
	...
	_html += '<input type="button" id="edit_menu_item" value="Edit Menu Item" disabled style="font-size: 9px;" title="Click this button to edit or modify the menu item that is highlighted in the list to the right of this button.  Menu Items differ from SubMenu Items in that a Menu Item is an individual item that appears on the Menu and a SubMenu Item is a collection of Menu Items.  You may Open a SubMenu to Edit the individual Menu Items by double-clicking the SubMenu or by clicking the appropriate button that appears below the list to the right of this button. SubMenu Items are displayed the same as Menu Items except the SubMenu Item appears enclosed by parenthesis." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuItemEditor(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '<tr>';
	...
	_html += '<input type="button" id="addit_menu_item" value="Add Menu Item" style="font-size: 9px;" title="Click this button to add a menu item.  Menu items are used to link to internal pages of content or to external documents such as Panagons docs or to external sites.  Menu items are different than SubMenu Items in that a Menu Item provides a single link to something whereas a SubMenu provides a container for a number of Menu Items." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuItemAdditor(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '<tr>';
	...
	_html += '<input type="button" id="addit_menu_submenu" value="Add Empty\nSubMenu" style="font-size: 9px;" title="Click this button to add a SubMenu Item.  SubMenu Items may contain one or more Menu Items and are used to extend the menu horizontally whereas Menu Items extend the Menu vertically." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuSubMenuAdditor(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</div>';
	_html += '</td>';
	_html += '</tr>';
	_html += '<tr>';
	...
	_html += '<input type="button" id="menu_browse_into" value="Open\nSub Menu" disabled style="font-size: 9px;" title="Click this button to Open a SubMenu, the SubMenu that is highlighted in the list directly above this button.  The other way to Open a SubMenu is to double-click the SubMenu item." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuBrowseInto(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '<td width="50%" align="right">';
	_html += '<input type="button" id="menu_browse_out" id="menu_browse_out" value="Close\nSub Menu" disabled style="font-size: 9px;" title="Click this button to Close a SubMenu that was previously opened." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuBrowseOut(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '</tr>';
	_html += '</table>';
	_html += '</td>';
	...
	_html += '</td>';
	_html += '<td align="center" valign="top">';
	_html += '<input type="button" id="menu_cut_to_clipboard" disabled value="Cut to\nClipboard" style="font-size: 9px;" title="Click this button to Cut a Menu Item or SubMenu Item to the ClipBoard. This function allows you to reorder the Menu; once a Menu Item or SubMenu is on the ClipBoard it may be Moved back to the Menu in a different position or into a different SubMenu container." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuCutToClipboard(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '<br><br>';
	_html += '<input type="button" id="menu_delete_item" disabled value="Delete\nItem" style="font-size: 9px;" title="Click this button to delete a Menu Item from the Menu.  You may also delete a SubMenu after the SubMenu is empty.  Use this button to delete each Menu Item from the SubMenu and then close the SubMenu to use this button to delete the empty SubMenu." onClick="var brObj = getGUIObjectInstanceById(\'menu_browser\'); if (brObj != null) { menuEditorObj.processMenuCutToThinAir(brObj, menuEditorObj.instances[' + menuEditorObj.id + ']) } return false;">';
	_html += '</td>';
	_html += '</tr>';
	_html += '</table>';
	_html += '</td>';
...
//	const_menu_copy_to_clipboard_symbol : menuEditorObj._const_menu_copy_to_clipboard_symbol, // this allows the copy to clipboard to be replaced by a delete item button without having to recode a lot of logic...
	...
	initMenuEditorClipboardObjects : function() {
		...
		if (!!rObj) {
			...
			html += '<tr>';
			html += '<td>';
			html += '<input type="button" id="clipboard_remove_item" value="Remove Clipboard Item" disabled style="font-size: 10px;" onClick="var clipObj = getGUIObjectInstanceById(\'menu_clipboard\'); if (!!clipObj) { menuEditorObj.processClipboardRemoveSelected(clipObj, menuEditorObj.instances[' + this.id + ']) } return false;">';
			html += '</td>';
			html += '</tr>';
		...
		if (!!eObj) {
			...
			if ( (isObjValidHTMLValueHolder(pListObj)) && (isObjValidHTMLValueHolder(pnListObj)) ) {
				...
				html += '<tr>';
				html += '<td align="left">';
				html += '<font size="1"><small><b>Prompt:</b></small></font>&nbsp;<input type="text" id="_menu_item_editor_prompt" size="39" maxlength="50" style="font-size: 9px;" onkeyup="menuEditorObj.refreshMenuItemEditorSaveButton(menuEditorObj.instances[' + this.id + ']); return true;">';
				html += '</td>';
				html += '</tr>';
...
// +++
				...
				html += '<input type="button" ' + _html + ' disabled value="[Save]" style="font-size: 9px;" title="Click this button to Save the Menu Item in the Menu.  Typically this button is used to Save any changes made to the Menu Item you may have been editing or adding to the Menu." onClick="menuEditorObj.processSaveMenuItemEditor(menuEditorObj.instances[' + this.id + ']); return false;">';
				html += '</td>';
				html += '<td>';
				html += '&nbsp;';
				html += '</td>';
				...
				html += '<input type="button" ' + _html + ' value="' + this.const_cancelButton_symbol + '" style="font-size: 9px;" title="Click this button to Cancel the operation currently in-process." onClick="menuEditorObj.processCancelMenuItemEditor(menuEditorObj.instances[' + this.id + ']); return false;">';
				html += '</td>';
				html += '</tr>';
				html += '</table>';
				html += '</td>';
