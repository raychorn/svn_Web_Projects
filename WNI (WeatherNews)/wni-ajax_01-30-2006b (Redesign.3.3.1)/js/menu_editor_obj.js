...
//	const_menu_copy_to_clipboard_symbol : menuEditorObj._const_menu_copy_to_clipboard_symbol, // this allows the copy to clipboard to be replaced by a delete item button without having to recode a lot of logic...
	...
	retrieveMenuByUUID : function(mm, id) {
		var i = -1;
		var toks = -1;
		for (i = 0; i < mm.length; i++) {
			if (typeof mm[i] == const_object_symbol) {
				this.menuSelectItemByPosition(i, id);
	...
	navigateToUUID : function(id) { // assumption: we always begin at the top of the menu...
		var c = this.retrieveMenuByUUID(this.menuArray, id);
	},
		...
		if (!!bObj) {
			...
			for (var i = (this._menu_stack.length == 0) ? 0 : 1; i < mm.length; i++) {
				if (!!mm[i]) {
					...
					if (_prompt.length > 0) {
						oObj = new Option( _prompt, _uuid); // this is the central point-of-focus for all actions on the menu itself...
