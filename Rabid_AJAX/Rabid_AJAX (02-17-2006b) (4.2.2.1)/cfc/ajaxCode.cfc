	...
	<cfscript>
		function enkrip2(aStruct) {
			...
			if (IsDefined("aStruct.plaintext")) {
				...
				aStruct.ciphertext = 'var enkripsi=[' & ticMark & ArrayToList(kode2, ',') & ticMark & '];';
		
				aStruct.decipher = obfuscateJScode(decipher_func()) & 'var _xx_ = decipher(enkripsi,' & ticMark &  aStruct.parameter & ticMark & ');';
