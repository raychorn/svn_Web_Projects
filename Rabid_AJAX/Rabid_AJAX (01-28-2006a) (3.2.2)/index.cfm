...
<!--- +++ --->
		...
		<!--//
			function _init() {
				var cObj = _getGUIObjectInstanceById('ta_jsCode');
				if (!!cObj) {
				//	alert('_init() :: cObj.value = [' + cObj.value + ']');

					try {
						eval(cObj.value); // the content passed-down from the server is JavaScript executable code.
					} catch(e) {
						jsErrorExplainer(e, '_init()', true);
					} finally {
					}
				}
	...
	// +++
		...
		<div id="div_jsCode" style="display: none;">
			<textarea id="ta_jsCode"></textarea>
		</div>
