...
AJAXEngine.prototype = {
	...
	//	this.createCSS();
		...
		var cObj = _getGUIObjectInstanceById(divName);
		if (!!cObj) {
	...
	createFrame : function() {  // define createFrame(); prototype
		...
		this.html += '<tr>';
		this.html += '<td>';
		var _style = 'width: ' + this.width + '; height: ' + this.height + '; left: ' + this.left + ' visibility: ' + this.visibility + '; background: cyan; ';
		if (document.layers) { 
			...
			this.html += '<iframe ' + sSrc + 'width="' + this.width + '" height="' + this.height + '" name="' + this.idGateway + '" id="' + this.idGateway + '" frameBorder="1" frameSpacing="0" marginWidth="0" marginHeight="0" scrolling="Auto" style="' + _style + '"></iframe>\n';
		}
		this.html += '</td>';
		this.html += '</tr>';
		this.html += '</table>';
