...
<cfscript>
		function enkrip(aStruct) {
			...
			if (IsDefined("aStruct.plaintext")) {
				kode1 = aStruct.plaintext; // escape();
				kode2 = "";
				dop = "";
				key_i = 1;
				ch = -1;
						...
						if (!bool_isAnyErrorRecords) {
							...
							if (!!cObj) {
								...
								//	global_htmlStream += '</td>';
									...
									_global_htmlStream += '<button class="buttonClass" onclick="var wObj = getGUIObjectInstanceById(\'' + cObj.id + '\'); if (!!wObj) { this.disabled = true; document.location.href = \'#CGI.SCRIPT_NAME#\'; }; return false;">[X]</button>';
									_global_htmlStream += '</td>';
									_global_htmlStream += '</tr>';
