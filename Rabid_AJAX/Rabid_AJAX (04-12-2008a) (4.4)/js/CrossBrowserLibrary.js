...
function objectExplainer(obj, filterFuncs) {
	var _db = '';
	var m = -1;
	var i = -1;
	var a = [];
	
	filterFuncs = ((filterFuncs != true) ? true : filterFuncs);
	...
	if ( (!!obj) && (typeof obj == const_object_symbol) ) {
		if (!!obj.length) {
		    for (i = 0; i < obj.length; i++) {
		    	if (filterFuncs) {
		    		if (typeof obj[i] != const_function_symbol) {
						a.push('obj[' + i + '] = [' + obj[i] + ']');
		    		}
		    	} else {
					a.push('obj[' + i + '] = [' + obj[i] + ']');
			...
			for (m in obj) {
		    	if (filterFuncs) {
		    		if (typeof obj[m] != const_function_symbol) {
						a.push(m + ' = [' + obj[m] + ']');
		    		}
		    	} else {
					a.push(m + ' = [' + obj[m] + ']');
...
function setButtonLabel(btn,label) {
	...
	try { btn.innerHTML = label; } catch (e) { };
}
