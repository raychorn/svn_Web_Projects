...
AJAXEngine = function(id, u, _d) {
	...
	this.browser_is_ff = ( (/Gecko/i.test(navigator.userAgent)) && (/Firefox/i.test(navigator.userAgent)) );
	this.browser_is_ie = ( ( (/msie/i.test(navigator.userAgent)) && (!this.browser_is_ff)  && !(/Netscape/i.test(navigator.userAgent)) && !(this.browser_is_opera) ) );
...
AJAXEngine.prototype = {
	...
	onTimeout : function() {   // define onTimeout(); callBack prototype
		this.throwError("(Warning) The current request has timed out.\nPlease try your request again.");
	},
	...
	receivePacket : function(packet, _bRunEvent) {  // define receivePacket(); prototype
		...
		clearInterval(this.timeoutID);  // stop waiting for a packet from the server...
		this.timeoutID = -1;
