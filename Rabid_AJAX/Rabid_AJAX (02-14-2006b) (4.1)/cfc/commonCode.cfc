		...
		function _populate_JS_queryObj(q, qObjName, aFunc, bool_asJScode) {
			...
			if (IsQuery(q)) {
				...
				for (i = 1; i lte q.recordCount; i = i + 1) {
					_jsCode = _jsCode & "if (!!#Request.parentKeyword##qObjName#) { #Request.parentKeyword##qObjName#.QueryAddRow(); " & Request.const_Cr;
					for (k = 1; k lte ListLen(cols, ','); k = k + 1) {
						...
						_jsCode = _jsCode & "#Request.parentKeyword##qObjName#.QuerySetCell('#cName#', #js_vVal#, #i#); " & Request.const_Cr;
					}
					_jsCode = _jsCode & ' }; ' & Request.const_Cr;
				}
