<cfcomponent displayname="ajaxCode" output="Yes" extends="commonCode">
	<cffunction name="js_jsInitCode" access="public" returntype="string">
		...
		<cfsavecontent variable="Request._jsInitCode">
			<cfoutput>if (!!#Request.___jsName___#) { } else { var #Request.___jsName___# = -1; }; #Request.___jsName___# = AJAXObj.getInstance(); _#Request.___jsName___# = new Object();</cfoutput>
		</cfsavecontent>
	...
	<cffunction name="js_jsCode_" access="public" returntype="string">
		...
		<cfsavecontent variable="Request._jsCode_">
			<cfoutput>#populate_JS_queryObj(Request.qryObj, '_#Request.___jsName___#.qParms', true)# if (!!#Request.___jsName___#) #Request.___jsName___#.init(); 
				<cfif (IsDefined("Request.qryData"))>
					<cfif (IsArray(Request.qryData))>
						...
						#populate_JS_queryObj(qStats, '_#Request.___jsName___#.qStats', true)# #Request.___jsName___#.push('qDataNum', _#Request.___jsName___#.qStats); 
						<cfloop index="_i" from="1" to="#n#">#populate_JS_queryObj(Request.qryData[_i], '_#Request.___jsName___#.qData#_i#', true)# #Request.___jsName___#.push('qData#_i#', _#Request.___jsName___#.qData#_i#); </cfloop>
					</cfif>
				</cfif>
				#Request.___jsName___#.push('qParms', _#Request.___jsName___#.qParms); _#Request.___jsName___# = null; 
			</cfoutput>
		</cfsavecontent>
	...
	<cfscript>
		function enkrip(aStruct) {
			...
			if (IsDefined("aStruct.plaintext")) {
				...
				for (i = 1; i lte panjang; i = i + 1)  {
				    if (LCASE(aStruct.metode) eq "kurang") {
						ch = chr(Asc(Mid(kode1, i, 1)) - Mid(aStruct.parameter, key_i, 1));
						if (asc(ch) eq 34) {
							ch = '\"';
						}
						kode2 = kode2 & ch;
						dop = "+";
				    } else if (LCASE(aStruct.metode) eq "tambah") {
						ch = chr(Asc(Mid(kode1, i, 1)) + Mid(aStruct.parameter, key_i, 1));
						if (asc(ch) eq 34) {
							ch = '\"';
						}
						kode2 = kode2 & ch;
						dop = "-";
					} else if (LCASE(aStruct.metode) eq LCASE(aStruct.const_metode_xor)) {
						ch = chr(BitXor(Asc(Mid(kode1, i, 1)), Mid(aStruct.parameter, key_i, 1)));
						if (asc(ch) eq 34) {
							ch = '\"';
						}
						kode2 = kode2 & ch;
				...
				aStruct.ciphertext = 'var enkripsi="' & kode2 & '";';
		
				dekripsinya = 'function decipher(enc, p){' & 'var teks=""; var teksasli="";' & 'var panjang=enc.length;' & 'var p_i=0;';
				dekripsinya = dekripsinya & 'for (var i=0;i<panjang;i++)' & '{ teks+=String.fromCharCode(enc.charCodeAt(i)' & dop & 'p.charAt(p_i)' & '); p_i++; if (p_i >= p.length) { p_i = 0; }; }';
				dekripsinya = dekripsinya & 'teksasli=unescape(teks);' & 'return teksasli;' & '};' & "xx = decipher(enkripsi,'"  &  aStruct.parameter  &  "');" & "alert(xx);";
				
				aStruct.decipher = dekripsinya;
				...
				aStruct.errorMsg = 'ERROR: Missing Variable known as aStruct.plaintext in function known as enkrip().';
			}
		}
		...
		function jsMinifier(jsIN) {
			...
			cf_file('WRITE', inName, jsIN);
			
			if (NOT Request.fileError) {
				cf_file('WRITE', cmdName, '"#exeName#" < "#inName#" > "#outName#"');

				if (NOT Request.fileError) {
					if ( (FileExists(exeName)) AND (FileExists(inName)) AND (FileExists(cmdName)) ) {
						cf_execute(cmdName, '', 10);
						writeOutput('3. <b>jsMinifier() :: exeName = [#exeName#], inName = [#inName#], Request.errorMsg = [#Request.errorMsg#]</b><br>');
					}
				} else {
					writeOutput('2. <b>jsMinifier() :: ERROR [#Request.errorMsg#]</b><br>');
				}
			} else {
				writeOutput('1. <b>jsMinifier() :: ERROR [#Request.errorMsg#]</b><br>');
			}
		}
		...
		function endAJAX() {
			...
			if (Request.bool_using_xmlHttpRequest) {
				...
				writeOutput(ReplaceList(obfuscateJScode(Request._jsCode_), Chr(13) & "," & Chr(10) & ",parent.", ",,"));
				...
				_jsCode = obfuscateJScode(_jsCode);
				jsMinifier(_jsCode);
