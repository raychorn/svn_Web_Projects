...
<!--- +++ --->
		...
		<!--//
			...
			function init() {
				...
				oAJAXEngine.create('div_ajaxSystem');
		...
		<!--
			...
			function queueUp_AJAX_Sessions3() {
				...
				execJSAPI_func('performEmailTest', 'displayEMailTestResults(' + oAJAXEngine.js_global_varName + ')');
			}
			
			function queueUp_AJAX_Sessions4() {
				pQueryString = '';
				oAJAXEngine.addNamedContext('#const_jsCompiler_symbol#', pQueryString);
				execJSAPI_func('performJsCompilerTest', 'displayJsCompilerResults(' + oAJAXEngine.js_global_varName + ')');
			}
						...
						if (!bool_isAnyErrorRecords) {
							...
							if (!!cObj) {
								...
								//	global_htmlStream += '</td>';
									...
									_global_htmlStream += '<button class="buttonClass" onclick="var wObj = getGUIObjectInstanceById(\'' + cObj.id + '\'); if (!!wObj) { this.disabled = true; wObj.disabled = true; flushGUIObjectChildrenForObj(wObj); wObj.innerHTML = \'\'; }; return false;">[X]</button>';
									_global_htmlStream += '</td>';
									_global_htmlStream += '</tr>';
			...
			function performEnkrip() {
				var iObj = getGUIObjectInstanceById('form_enkrip_input_length');
				var oObj = getGUIObjectInstanceById('form_enkrip_enkrip_length');
				var dObj = getGUIObjectInstanceById('form_enkrip_diff_length');
				var aFormObj = getGUIObjectInstanceById('form_enkrip');
			...
			function performEnkripExec() {
				var tObj = getGUIObjectInstanceById('hasil');
				if (!!tObj) {
					try {
						eval(tObj.value);
					} catch(ee) {
				...
				if (nRecs > 0) { // at present only the first data record is consumed...
					...
					if (!!qData) {
						...
						//	_alert(oParms);
							...
							var cObj = getGUIObjectInstanceById('div_ajaxSystem');
							if (!!cObj) {
								...
								var cObj2 = getGUIObjectInstanceById('kodeawal');
								if (!!cObj2) {
		...
		<div id="div_ajaxSystem" style="display: inline;">
		</div>
