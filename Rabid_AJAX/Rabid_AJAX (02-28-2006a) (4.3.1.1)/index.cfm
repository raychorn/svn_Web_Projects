...
<cfif 0>
	...
	<div id="div_ajaxSystem" style="position: absolute;"></div>
		...
		<!--
			...
			var global_testNum = 0;
			var global_num_to_watch = [];
			var global_msBegin = [];
			var global_msEnd = [];
			global_num_to_watch[global_testNum] = -1;
			
			// 90000 ms is the time limit for SDA use... approx. 68 seconds
			
			function computeTemporalBias() {
				// This method of computing the temporal bias takes 2.25 minutes before a change is noticed...
				var dt = new Date();
				var tP = dt.getTime() / 90000;
				var ar = tP.toString().split('.');
				var tP2 = parseInt(ar[0]);
				if (global_num_to_watch[global_testNum] == -1) {
					global_num_to_watch[global_testNum] = tP2;
					global_msBegin[global_testNum] = dt.getTime();
				}
				if (global_num_to_watch[global_testNum] != tP2) {
					global_msEnd[global_testNum] = dt.getTime();
					_alert('[' + global_testNum + '] msDiff = [' + (global_msEnd[global_testNum] - global_msBegin[global_testNum]) + ']');
					global_testNum++;
					if (global_testNum == 100) {
						clearInterval(tid);
					}
					global_num_to_watch[global_testNum] = -1;
				}
			//	var _db = dt + ', time = [' + dt.getTime() + ']' + ', tP = [' + tP + ']' + ', tP2 = [' + tP2 + ']';
			//	_alert(_db);
			}
			
			var tid = setInterval("computeTemporalBias()", 1000);
		// -->
		</script>
