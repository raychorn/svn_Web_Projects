		...
		<!--
			function _dispaySysMessages(s, t, bool_hideShow) {
				...
				if ( (!!cObj) && (!!sObj) && (!!tObj) ) {
					...
					sObj.innerHTML = '<textarea class="code" cols="150" rows="30" readonly>' + s + '</textarea>';
					tObj.innerHTML = t;
...
// +++
			...
			function onChange_EventHandler(oObj) {
				...
				if ( (!!oObj) && (!!oObj.options) && (!!oObj.selectedIndex) ) {
					...
					global_maxRecsPerPage = parseInt(aValue);
			...
			function displayEditorControlPanel() {
				...
				if (oGateway.currentContextName == oGateway.namedContextStack[0]) {
					...
					_html += '<option value="" ' + ((global_maxRecsPerPage > 0) ? '' : 'SELECTED') + '>';
					_html += 'Choose...';
					_html += '</option>';
	
					_html += '<option value="5"' + ((global_maxRecsPerPage == 5) ? 'SELECTED' : '') + '>';
					_html += '5';
					_html += '</option>';
	
					_html += '<option value="10"' + ((global_maxRecsPerPage == 10) ? 'SELECTED' : '') + '>';
					_html += '10';
					_html += '</option>';
			...
			function advanceIntraPageNav(offset, aContextName) {
				function hideShowRecordContainers(aPageNum, bool_hideOrShow) {
					...
					if ( (!!aPageNum) && (typeof aPageNum != const_object_symbol) ) {
						...
						for (i = 1; i <= global_maxRecsPerPage; i++) {
			...
			function displayTopLevelMenuNames(qObj) {
				...
				function makeRecordLink(sData, spanName, recID) {
					var _html = '';
					
					_html += '<a href="" onclick="clickEventHandler_Record(\'' + sData + '\',\'' + spanName + '\',\'' + recID + '\'); return false;">' + sData + '</a>';
				...
				function displayRecord(_ri, _dict, _rowCntName) {
					...
					_html += '<td style="' + _rowStyle + '"><span id="span_' + _spanName + '_' + _col2_keyName + '_' + _ri + '" class="normalStatusClass">' + makeRecordLink(_dataVal, _spanName, _ID) + '</span></td>'; // this is the data column...
					...
					if (global_maxRecsPerPage > 0) {
						if (i_recordContainerCount == global_maxRecsPerPage) {
				...
				if (nRecs > 0) { // at present only the first data record is consumed...
					...
					if (!!qData) {
						...
						_html += '</table>';
						displayTable(_html, '*', const_paper_color_light_yellow, 1);
						...
						if (!bool_isAnyErrorRecords) {
							...
							if (!!cObj) {
								...
								if (!cObj2b) {
									...
									global_htmlStream += '<div id="div_contents_#const_menuRecord_symbol#">(2)</div>';
									global_htmlStream += '</td>';
									global_htmlStream += '</tr>';
									global_htmlStream += '<tr>';
									...
									global_htmlStream += '</td>';
									global_htmlStream += '<td valign="top" align="center">';
									global_htmlStream += '<div id="div_contents_#const_groupNameRecord_symbol#">(3)</div>';
									global_htmlStream += '</td>';
									global_htmlStream += '</tr>';
							...
							// BEGIN: Pop-Up an error message here...
							// END! Pop-Up an error message here...
						}
...
// +++
			...
			function performAssociateMenu2Group() {
				...
				if ( (!!cObj1) && (!!cObj2) ) {
					menuName = cObj1.innerHTML.stripHTML().trim();
					groupName = cObj2.innerHTML.stripHTML().trim();
					alert('performAssociateMenu2Group() :: ' + 'menuName = [' + menuName + ']' + ', groupName = [' + groupName + ']' + '\nglobal_dict = [' + global_dict + ']');
...
// +++
			function performSendJsToServer() {
				var i = -1;
				var ar = [];
				var _jsCode = '';
				var cObj = getGUIObjectInstanceById('textarea_jsCode');
				if (!!cObj) {
					_jsCode = cObj.value;
					oGateway.currentContextName = ''; // make sure there is no interference from other contexts...
					// split the code into small chunks BEFORE calling AJAX because CF has problems dealing with this part of the process...
					ar = _jsCode.split('\n');
					for (i = 0; i < ar.length; i++) {
						ar[i] = ar[i].URLEncode();;
					}
					// +++ add code to accept an array and have execJSAPI_func() make the contents of the array into a series of arguments...
					execJSAPI_func('encryptJSCode', 'displayJsCode(' + oGateway.js_global_varName + ')', ar);
				//	alert('performSendJsToServer() :: \n' + ar);
				}
			}
			
			function displayJsCode(qObj) {
				var nRecs = -1;
				...
				if (nRecs > 0) { // at present only the first data record is consumed...
					...
					if (!!qData) {
						...
						var _db = qData.toString();
						dispaySysMessages(_db, 'DEBUG');
					//	alert(qData);
					}
				}
			}
...
// +++
							...
							_html += '<hr width="80%" color="blue">';
							_html += '<textarea id="textarea_jsCode" cols="80" rows="15" class="textClass"></textarea>';
							_html += '<br>';
							_html += '<button class="buttonClass" onclick="performSendJsToServer(); return false;">[Try It]</button>';
							_html += '';
							_html += '';
...
// +++				
		...
		<b>Welcome to the Rabid_AJAX - The Next BIG Thing on the Internet !</b>
		<br><br>
