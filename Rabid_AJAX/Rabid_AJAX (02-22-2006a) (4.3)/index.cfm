...
<cfif 0>
	...
	<div id="div_ajaxSystem" style="position: absolute;"></div>
		...
		<!--
			...
			var global_num_to_watch = -1;
			
			function computeTemporalBias() {
				// This method of computing the temporal bias takes 2.25 minutes before a change is noticed...
				var dt = new Date();
				var tP = dt.getTime() / 1000;
				var ar = tP.toString().split('.');
				var tP2 = parseInt(ar[0]) / 1000;
				var ar2 = tP2.toString().split('.');
				var tP3 = parseInt(ar2[0]) / 10;
				var ar3 = tP3.toString().split('.');
				var tP4 = parseInt(ar3[1]); // this is the temporal bias...
				if (global_num_to_watch == -1) {
					global_num_to_watch = tP4;
				}
				if (global_num_to_watch != tP4) {
					clearInterval(tid);
				}
				var _db = dt + ', ms = [' + dt.getMilliseconds() + ']' + ', time = [' + dt.getTime() + ']' + ', tP = [' + tP + ']' + ', tP2 = [' + tP2 + ']' + ', tP3 = [' + tP3 + ']' + ', tP4 = [' + tP4 + ']';
				_alert(_db);
			}
