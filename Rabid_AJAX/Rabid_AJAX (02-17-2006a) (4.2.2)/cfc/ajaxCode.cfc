<cfcomponent displayname="ajaxCode" output="Yes" extends="commonCode">
	...
	<cfscript>
		function enkrip(aStruct) {
			...
			if (IsDefined("aStruct.plaintext")) {
				...
				for (i = 1; i lte panjang; i = i + 1)  {
				    if (LCASE(aStruct.metode) eq "kurang") {
						...
						if ( (asc(ch) eq 34) OR (asc(ch) eq 37) OR (asc(ch) eq 39) ) {
						//	ch = URLEncodedFormat(ch);
						}
						kode2 = kode2 & ch;
						dop = "+";
				...
				aStruct.ciphertext = 'var enkripsi=' & ticMark & URLEncodedFormat(kode2) & ticMark & ';';
			//	aStruct.ciphertext = 'var enkripsi=' & '"' & kode2 & '"' & ';';
			//	aStruct.ciphertext = 'var enkripsi=' & ticMark & kode2 & ticMark & ';';
		
				dekripsinya = 'function decipher(enc, p){ var teks=' & ticMark2 & '; var teksasli=' & ticMark2 & '; enc=enc.URLDecode(); alert(enc); var panjang=enc.length; var p_i=0;';
			...
			//	dekripsinya = dekripsinya & 'teksasli=unescape(teks);' & 'return teksasli;' & '};' & 'var _xx_ = decipher(enkripsi,' & ticMark &  aStruct.parameter & ticMark & ');';
				dekripsinya = dekripsinya & 'return teks;' & '};' & 'var _xx_ = decipher(enkripsi,' & ticMark &  aStruct.parameter & ticMark & ');';
		...
		function enkrip2(aStruct) {
			...
			if (IsDefined("aStruct.plaintext")) {
				...
				for (i = 1; i lte panjang; i = i + 1)  {
					...
					kode2[ArrayLen(kode2) + 1] = ch; // num2Hex(ch);
					key_i = key_i + 1;
					if (key_i gt Len(key)) {
						key_i = 1;
					}
				...
				aStruct.ciphertext = 'var enkripsi=[' & ArrayToList(kode2, ',') & '];';
		
				dekripsinya = 'function decipher(enc, p){ var teks=' & ticMark2 & '; var panjang=enc.length; var p_i=0;';
				dekripsinya = dekripsinya & 'for (var i=0;i<panjang;i++){ teks+=String.fromCharCode(enc[i]' & dop & 'p.charAt(p_i)' & '); p_i++; if (p_i >= p.length) { p_i = 0; }; }';
				dekripsinya = dekripsinya & 'return teks.URLDecode();' & '};' & 'var _xx_ = decipher(enkripsi,' & ticMark &  aStruct.parameter & ticMark & ');';
		...
		function endAJAX() {
			...
			if (Request.bool_using_xmlHttpRequest) {
				...
				writeOutput('/* BOF CFAJAX */' & jsMinifier(_jsCode) & '/* EOF CFAJAX */');
			} else {
				...
				_jsCode = _jsCode & 'eval(_xx_);';
			//	_jsCode = _jsCode & 'alert(enkripsi.length);';
